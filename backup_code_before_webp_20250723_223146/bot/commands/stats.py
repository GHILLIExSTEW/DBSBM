...
class ChannelSelect(Select):
    ...
    async def callback(self, interaction: Interaction):
        try:
            ...
            if not stats_data:
                await interaction.followup.send(
                    "❌ Stats data not found. Please select a capper/server first.",
                    ephemeral=True,
                )
                return
            ...
            image_generator = (
                StatsImageGenerator()
            )  # Assumes StatsImageGenerator doesn't need bot/db
            if is_server:
            ...
            img_buffer.seek(0)
            file = File(img_buffer, filename="stats.webp")
            ...
            if img_buffer:
                ...
                await interaction.followup.send(
                    f"✅ Statistics image sent to {channel.mention}", ephemeral=True
                )
            else:
...
class StatsView(View):
    ...
    async def update_message(
        ...
        try:
            ...
            logger.warning(
                "Original interaction message for StatsView not found (timed out or deleted)."
            )
        except Exception as e:
    ...
    async def populate_cappers(self):
        ...
        try:
            ...
            for capper in cappers:
                if fetched_users >= 24:
                    break  # Limit options in Select menu (25 total max)
                user = self.original_interaction.guild.get_member(
                    capper["user_id"]
                )  # Try cache first
                if not user:  # Fetch if not in cache
                    try:
                        user = await self.original_interaction.guild.fetch_member(
                            capper["user_id"]
                        )
                    except discord.NotFound:
                        continue  # Skip if member left
                    except discord.Forbidden:
                        continue  # Skip if cannot fetch
                    except Exception as e:
                        logger.warning(
                            f"Error fetching member {capper['user_id']}: {e}"
                        )
                        continue
                if user:
                    options.append(
            ...
            self.capper_select = Select(
                placeholder="Select a Capper or 'Server Stats'", options=options
            )
            ...
            logger.exception(
                f"Error populating cappers for guild {self.original_interaction.guild_id}: {e}"
            )
            # Try to send error message
            if not self.original_interaction.response.is_done():
                await self.original_interaction.response.send_message(
                    "❌ Error fetching cappers.", ephemeral=True
                )
            else:
    ...
    async def capper_select_callback(self, interaction: Interaction):
        ...
        try:
            ...
            if selection == "server":
                self.stats_data = await self.analytics_service.get_guild_stats(
                ...
                self.stats_data = await self.analytics_service.get_user_stats(
                ...
                if capper_info:
                    username = capper_info.get(
                        "display_name", f"User {self.selected_user_id}"
                    )
                    ...
                    ) or await interaction.guild.fetch_member(self.selected_user_id)
                    username = (
                        user.display_name if user else f"User {self.selected_user_id}"
                    )
                    profile_image_url = None
                    self.selected_username = username
                    self.selected_profile_image_url = None
            ...
            if not self.stats_data:
                await interaction.followup.send(
                    "❌ Could not retrieve statistics data.", ephemeral=True
                )
                self.stop()
                return
            ...
            if self.is_server:
                ...
                img = (
                    await loop.run_in_executor(
                        None,
                        lambda: image_generator.generate_guild_stats_image(
                            self.stats_data
                        ),
                    )
                    if loop
                    else image_generator.generate_guild_stats_image(self.stats_data)
                )
                img_buffer = BytesIO()
                img.save(img_buffer, format="PNG")
                img_buffer.seek(0)
...
class StatsCog(commands.Cog):
    ...
    @app_commands.command(
        name="stats", description="View betting statistics for cappers or the server."
    )
    ...
    async def stats(self, interaction: Interaction):
        ...
        try:
            ...
            if not hasattr(self.bot, "db_manager"):
                ...
                await interaction.response.send_message(
                    "❌ Bot is not properly configured (DB Error).", ephemeral=True
                )
                return
            ...
            logger.exception(
                f"Error initiating stats command for {interaction.user}: {e}"
            )
            if not interaction.response.is_done():
                await interaction.response.send_message(
                    "❌ An error occurred while starting the stats command.",
                    ephemeral=True,
                )
