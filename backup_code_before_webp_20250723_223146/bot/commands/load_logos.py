...
try:
    ...
    if os.path.exists(dotenv_path):
        ...
        logger.warning(
            f".env file not found at: {dotenv_path}. Relying on pre-set env vars."
        )
    ...
    if not TEST_GUILD_ID_STR:  # AUTH_USER_ID can be optional if check is elsewhere
        logger.warning(
            "TEST_GUILD_ID not set. load_logos command may be restricted or fail."
        )
        TEST_GUILD_ID = None
    else:
    ...
    if not AUTHORIZED_USER_ID_STR:
        logger.warning(
            "AUTHORIZED_LOAD_LOGO_USER_ID not set. Command authorization will rely on permissions only."
        )
...
def process_and_save_logo(
    ...
    is_league: bool,
    league_code_for_path: Optional[
        str
    ] = None,  # The league code, e.g., "NHL", "NCAAF", "NBA"
) -> Optional[str]:
    try:
        with Image.open(BytesIO(logo_bytes)) as img:
            ...
            current_script_commands_dir = os.path.dirname(
                os.path.abspath(__file__)
            )  # .../betting-bot/commands
            betting_bot_dir = os.path.dirname(
            ...
            if is_league:
                ...
                # Path: {final_save_root}/logos/leagues/{SPORT_CATEGORY_FROM_MAP}/{league_code_lower}.webp
                ...
                save_filename = f"{league_code_lower}.webp"
            else:
                # Saving a Team Logo
                if not league_code_for_path:
                    logger.error(
                        f"league_code_for_path (e.g., NHL, NCAAF) required for team logo '{name_to_save}'."
                    )
                    return None
                ...
                if league_code_upper.startswith("NCAA"):
                    specific_sport_for_ncaa = get_sport_category_for_path(
                        league_code_upper
                    )
                    if (
                        specific_sport_for_ncaa == "OTHER_SPORTS"
                    ):  # Default fallback from helper
                        logger.warning(
                            f"NCAA league '{league_code_upper}' not in SPORT_CATEGORY_MAP, using 'UNKNOWN_NCAA_SPORT'."
                        )
                        specific_sport_for_ncaa = "UNKNOWN_NCAA_SPORT"
                    # Path: {final_save_root}/logos/teams/NCAA/{SpecificSportLikeFootball}/
                    target_dir = os.path.join(
                        logos_base_dir, "teams", "NCAA", specific_sport_for_ncaa
                    )
                else:
                    # Standard leagues
                    ...
                    target_dir = os.path.join(
                        logos_base_dir, "teams", sport_category, league_code_upper
                    )

                # team name (name_to_save) becomes the filename base
                save_filename = f"{safe_filename_base}.webp"
...
class LoadLogosCog(commands.Cog):
    ...
    @app_commands.command(
        name="load_logos", description="Load team or league logos (Restricted User)."
    )
    ...
    async def load_logos_command(
        ...
        if (
            AUTHORIZED_USER_ID and interaction.user.id != AUTHORIZED_USER_ID
        ):  # Check if AUTH_USER_ID is set before comparing
        ...
        if not is_league and not league_code:
            await interaction.response.send_message(
                "❌ `league_code` (e.g., NHL, NCAAF) is required for TEAM logos.",
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            if not ct or not ct.startswith("image/"):
                await interaction.followup.send(
                    f"❌ Invalid file type ({ct}). Upload PNG, JPG, GIF, WEBP.",
                    ephemeral=True,
                )
                return
            ext = (".webp", ".jpg", ".jpeg", ".gif", ".webp")
            if not logo_file.filename.lower().endswith(ext):
                await interaction.followup.send(
                    f"❌ Invalid file extension. Use: {', '.join(ext)}.", ephemeral=True
                )
                return
            ...
            if saved_path:
                ...
                await interaction.followup.send(
                    f"❌ Failed to process logo for **{name}**. Check logs.",
                    ephemeral=True,
                )
...
async def setup(bot: commands.Bot):
    if not TEST_GUILD_ID:
        logger.warning(
            "LoadLogosCog not loaded: TEST_GUILD_ID missing or invalid in .env."
        )
        return
