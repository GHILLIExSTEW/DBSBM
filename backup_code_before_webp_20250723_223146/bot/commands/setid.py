...
class CapperModal(Modal, title="Capper Profile Setup"):
    ...
    async def on_submit(self, interaction: Interaction):
        try:
            ...
            logger.info(
                f"Capper entry created/exists for {user_id} in guild {guild_id}."
            )

            # Follow up - ask for image
            image_view = ImageUploadView(
                guild_id=guild_id, user_id=user_id, db_manager=self.db
            )
            await interaction.followup.send(
            ...
            await interaction.followup.send(
                "❌ An error occurred while setting up the profile.", ephemeral=True
            )
    ...
    async def on_error(self, interaction: Interaction, error: Exception) -> None:
        ...
        await interaction.followup.send(
            "❌ An error occurred with the profile setup modal.", ephemeral=True
        )
...
class ImageUploadView(View):
    ...
    @button(
        ...
        custom_id="capper_provide_url",
    )
    async def provide_url_button(self, interaction: Interaction, button: Button):
        modal = ImageURLModal(
            guild_id=self.guild_id, user_id=self.user_id, db_manager=self.db
        )
        ...
        try:
            ...
            with Image.open(image_data) as img:
                ...
                try:
                    ...
                    return
                url_path = f"/static/guilds/{guild_id}/users/{user_id}.webp"
                ...
                await interaction.followup.send(
                    "✅ Profile picture updated successfully!", ephemeral=True
                )
        except Exception as e:
...
class ImageURLModal(Modal, title="Enter Profile Image URL"):
        ...
        try:
            ...
            try:
                ...
                if not content_type.startswith("image/"):
                    await interaction.followup.send(
                        "❌ URL does not point to a valid image type.", ephemeral=True
                    )
                    return
                ...
                logger.error(
                    f"Failed to download image from URL {image_url}: {req_err}"
                )
                await interaction.followup.send(
                    "❌ Failed to download image from URL. Check the link.",
                    ephemeral=True,
                )
                return
                ...
                await interaction.followup.send(
                    "❌ Could not process the image file from the URL.", ephemeral=True
                )
                return
            except Exception as proc_err:
                logger.exception(f"Error processing image from {image_url}: {proc_err}")
                await interaction.followup.send(
                    "❌ An error occurred while processing the image.", ephemeral=True
                )
                return
            ...
            if not interaction.response.is_done():
                ...
                await interaction.followup.send(
                    "❌ An error occurred processing the image URL.", ephemeral=True
                )
    ...
    async def on_error(self, interaction: Interaction, error: Exception) -> None:
        ...
        if not interaction.response.is_done():
            await interaction.response.send_message(
                "❌ An error occurred with the URL modal.", ephemeral=True
            )
        else:
            await interaction.followup.send(
                "❌ An error occurred with the URL modal.", ephemeral=True
...
class SetIDCog(commands.Cog):
    ...
    @app_commands.command(
        name="setid", description="Set your capper ID and display name."
    )
