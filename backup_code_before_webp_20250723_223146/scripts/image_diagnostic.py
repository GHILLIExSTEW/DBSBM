...
class ImageDiagnostic:
        ...
        try:
            with Image.open(file_path) as img:
                ...
                if img.format != "PNG":
                    self.stats["other_errors"] += 1
                    self.failed_files.append(str(file_path))
                    self.error_details[str(file_path)] = (
                        f"Not a PNG file (format: {img.format})"
                    )
                    return False
    ...
    def run_diagnostic(self):
        ...
        # Find all PNG files
        png_files = list(self.target_path.rglob("*.webp"))
    ...
    def print_results(self):
        ...
        logger.info(
            f"Missing conversions: {expected_conversions - actual_conversions - failed_conversions}"
        )
