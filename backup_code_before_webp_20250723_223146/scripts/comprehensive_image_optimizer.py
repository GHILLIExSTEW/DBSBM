...
class ComprehensiveImageOptimizer:
    def __init__(self, dry_run: bool = False, quality: int = 85, target_path: str = "bot/static/logos"):
        ...
        self.supported_formats = {'.webp', '.jpg', '.jpeg', '.gif', '.bmp', '.tiff', '.tif'}
    ...
    def convert_to_webp(self, image_path: Path) -> bool:
        ...
        try:
            ...
            if not self.dry_run:
                ...
                self.log_action(f"Converted {actual_format.upper()} to WebP", str(image_path), size_saved)
                ...
                self.log_action(f"Would convert {actual_format.upper()} to WebP", str(image_path))
    ...
    def optimize_directory(self, directory: Path) -> int:
        ...
        for file_path in all_files:
            if file_path.is_file() and file_path.suffix.lower() not in self.supported_formats:
...
def main():
    parser = argparse.ArgumentParser(description="DBSBM Comprehensive Image Optimization Tool")
    ...
    try:
        ...
        if not args.dry_run and result['files_converted'] > 0:
            logger.info(f"🎉 Converted {result['files_converted']} files to WebP format")
