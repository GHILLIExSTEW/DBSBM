...
class WebPCodeUpdater:
    def __init__(self, dry_run: bool = False, create_backup: bool = True):
        ...
        self.patterns = [
            ...
            (r'\.png(?=\s*[,\s\)\]\}\n])', '.webp'),  # .webp followed by whitespace, comma, or closing brackets
            ...
            (r'Image\.open\([^)]*\.png[^)]*\)', lambda m: m.group(0).replace('.webp', '.webp')),
            (r'img\.save\([^)]*\.png[^)]*\)', lambda m: m.group(0).replace('.webp', '.webp')),
            ...
            (r'os\.path\.exists\([^)]*\.png[^)]*\)', lambda m: m.group(0).replace('.webp', '.webp')),
            (r'Path\([^)]*\.png[^)]*\)\.exists\(\)', lambda m: m.group(0).replace('.webp', '.webp')),
        ]
