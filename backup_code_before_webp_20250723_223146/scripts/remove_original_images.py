...
class OriginalImageRemover:
    def __init__(self, target_path: str = "bot/static", dry_run: bool = False, 
                 create_backup: bool = True, verify_webp: bool = True):
        ...
        self.original_formats = ['.webp', '.jpg', '.jpeg', '.gif', '.bmp', '.tiff', '.tif']
    ...
    def find_original_images(self) -> List[Path]:
        ...
        for file_path in all_files:
            if file_path.is_file() and file_path.suffix.lower() not in self.original_formats:
...
def main():
    parser = argparse.ArgumentParser(description="Remove original image files after WebP conversion")
    ...
    parser.add_argument("--no-verify", action="store_true", help="Skip WebP existence verification")
