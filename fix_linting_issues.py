...
def fix_community_analytics():
    ...
    content = re.sub(
        r'f"Failed to track metric: \{e\}"', f'"Failed to track metric: {e}"', content
    )
...
def fix_community_events():
    ...
    # Fix imports
    content = re.sub(
        r"from typing import Dict, Optional", "from typing import Optional", content
    )
...
def fix_community_commands():
    ...
    # Fix f-string without placeholders
    content = re.sub(
        r'f"Community discussion started by \{interaction\.user\.id\}"',
        '"Community discussion started by {interaction.user.id}"',
        content,
    )
