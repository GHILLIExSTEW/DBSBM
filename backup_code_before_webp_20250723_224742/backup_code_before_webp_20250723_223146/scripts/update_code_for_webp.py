...
1. Updates file extension references from .png to .webp
...
class WebPCodeUpdater:
    def __init__(self, dry_run: bool = False, create_backup: bool = True):
        ...
        self.patterns = [
            ...
            (r'\.png(?=\s*[,\s\)\]\}\n])', '.webp'),  # .png followed by whitespace, comma, or closing brackets
            (r'\.png(?=\s*["\'])', '.webp'),  # .png followed by quotes
            (r'\.png(?=\s*[;])', '.webp'),  # .png followed by semicolon
            ...
            (r'Image\.open\([^)]*\.png[^)]*\)', lambda m: m.group(0).replace('.png', '.webp')),
            (r'img\.save\([^)]*\.png[^)]*\)', lambda m: m.group(0).replace('.png', '.webp')),
            ...
            (r'os\.path\.exists\([^)]*\.png[^)]*\)', lambda m: m.group(0).replace('.png', '.webp')),
            (r'Path\([^)]*\.png[^)]*\)\.exists\(\)', lambda m: m.group(0).replace('.png', '.webp')),
        ]
        ...
        self.target_extensions = ['.py', '.html', '.js', '.css', '.md', '.txt', '.json', '.yaml', '.yml']
