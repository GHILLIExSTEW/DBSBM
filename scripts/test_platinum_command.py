...
async def test_platinum_command():
    ...
    try:
        ...
        export_count = await platinum_service.get_export_count(
            test_guild_id, datetime.now().month
        )
        logger.info(f"Export count (this month): {export_count}")

        # Test 4: Check subscription service
        subscription_level = await subscription_service.get_subscription_level(
            test_guild_id
        )
        ...
        if is_platinum:
            ...
            print(
                "💡 Guild needs upgrade - will show upgrade message with $99.99 pricing"
            )
