...
class TeamsAndPlayersCompiler:
    ...
    async def get_players_for_team(self, league: str, team: str) -> List[str]:
        ...
        try:
            ...
            players_data = (
                await self.player_search_service._get_players_from_team_library(
                    league, team
                )
            )
            player_names = [
            ...
            if not player_names:
                if league == "NBA":
                    player_names = (
                        await self.player_search_service._get_nba_team_players(team)
                    )
                elif league == "NFL":
                    player_names = (
                        await self.player_search_service._get_nfl_team_players(team)
                    )
                elif league == "MLB":
                    player_names = (
                        await self.player_search_service._get_mlb_team_players(team)
                    )
                elif league == "NHL":
                    player_names = (
                        await self.player_search_service._get_nhl_team_players(team)
                    )
                elif league in ["EPL", "LaLiga", "Bundesliga", "SerieA", "Ligue1"]:
                    player_names = (
                        await self.player_search_service._get_soccer_team_players(team)
                    )
