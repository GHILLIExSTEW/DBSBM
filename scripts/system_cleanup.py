...
class SystemCleanup:
    def __init__(self, dry_run: bool = False, backup_count: int = 3, cache_size_limit: int = 1024):
    ...
    def cleanup_duplicate_documentation(self) -> int:
        ...
        if len(existing_files) > 1:
            ...
            for file_path in existing_files:
                ...
                logger.warning(f"  - {file_path.name} ({size} bytes, modified {mtime.strftime('%Y-%m-%d %H:%M')})")
...
def main():
    ...
    parser.add_argument("--cache-size-limit", type=int, default=1024,
