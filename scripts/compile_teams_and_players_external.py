...
class ExternalPlayerSearcher:
    ...
    def search_google(self, query: str) -> Optional[str]:
        ...
        try:
            ...
            for link in soup.find_all('a', href=True):
                href = link['href']
                if any(domain in href for domain in ['espn.com', 'sports-reference.com', 'basketball-reference.com', 'baseball-reference.com', 'hockey-reference.com', 'pro-football-reference.com']):
                    if href.startswith('/url?q='):
                        href = href.split('/url?q=')[1].split('&')[0]
                    return href
    ...
    def extract_players_from_wikipedia(self, url: str, team_name: str, league: str) -> List[str]:
                ...
                if any(keyword in header_text for keyword in ['player', 'name', 'roster', 'squad']):
                    ...
                    for row in rows:
                        ...
                        if len(cells) >= 2:
                            ...
                            player_name = re.sub(r'\[.*?\]', '', player_name)  # Remove Wikipedia links
                            ...
                            if (len(player_name) > 2 and 
                                ...
                                not any(word in player_name.lower() for word in ['total', 'coach', 'manager', 'captain', 'goalkeeper', 'defender', 'midfielder', 'forward'])):
    ...
    def extract_players_from_sports_site(self, url: str, team_name: str, league: str) -> List[str]:
    ...
    def search_team_roster(self, team_name: str, league: str) -> tuple[List[str], Optional[str]]:
        ...
        for query in search_queries:
            ...
            if wiki_url:
                players = self.extract_players_from_wikipedia(wiki_url, team_name, league)
                if players:
                    logger.info(f"Found {len(players)} players from Wikipedia for {team_name}")
            ...
            if google_url:
                if 'wikipedia.org' in google_url:
                    players = self.extract_players_from_wikipedia(google_url, team_name, league)
                else:
                    players = self.extract_players_from_sports_site(google_url, team_name, league)
                
                if players:
                    logger.info(f"Found {len(players)} players from external source for {team_name}")
