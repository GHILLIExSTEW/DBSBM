#!/usr/bin/env python3
"""
Test MLB API to find the correct season and parameters.
"""
...
async def test_mlb_api():
    ...
    headers = {
        ...
        'User-Agent': 'DBSBM-MLBTest/1.0'
    }
    
    async with aiohttp.ClientSession(headers=headers) as session:
        # Get MLB league info
        league_info = LEAGUE_IDS.get('MLB')
        if not league_info:
            print("❌ MLB league info not found!")
            return
        
        sport = league_info['sport']
        league_id = league_info['id']
        
        print(f"MLB League Info:")
        print(f"  Sport: {sport}")
        ...
        seasons_to_test = [2024, 2023, 2022]
