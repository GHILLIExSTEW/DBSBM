...
class ComprehensivePlayerSearcher:
    ...
    def _extract_players_from_wikipedia_page(self, soup: BeautifulSoup) -> List[str]:
        ...
        for table in tables:
            ...
            if any(
                ...
                for keyword in ["player", "name", "roster", "squad", "current"]
            ):
    ...
    def search_team_roster(
        ...
        if manual_players:
            logger.info(
                f"Found {len(manual_players)} players from manual data for {team_name}"
            )
        ...
        if wiki_players:
            logger.info(
                f"Found {len(wiki_players)} players from Wikipedia for {team_name}"
            )
