    ...
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                # Check darts games
                await cur.execute(
                    "SELECT COUNT(*) FROM api_games WHERE sport = 'Darts'"
                )
                ...
                # Check tennis games
                await cur.execute(
                    "SELECT COUNT(*) FROM api_games WHERE sport = 'Tennis'"
                )
                ...
                # Check esports games
                await cur.execute(
                    "SELECT COUNT(*) FROM api_games WHERE sport = 'Esports'"
                )
                ...
                for game in sample_darts:
                    logger.info(
                        f"  {game[0]}: {game[1]} vs {game[2]} ({game[3]}) - {game[4]}"
                    )

                # Show all sports in database
                await cur.execute(
                    "SELECT DISTINCT sport, COUNT(*) FROM api_games GROUP BY sport ORDER BY COUNT(*) DESC"
                )
