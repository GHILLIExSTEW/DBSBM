...
class APIEndpointsAnalyzer:
    ...
    def extract_endpoints_from_file(self, file_path: Path) -> Dict:
        ...
        api_name = (
            file_path.stem.replace("API_Sports___Documentation_", "")
            .replace("API_", "")
            .replace("___", " ")
        )
    ...
    def extract_description(self, section: str) -> str:
        ...
        desc_patterns = [r"([A-Z][^.]*\.)", r"Get ([^.]*\.)", r"Returns ([^.]*\.)"]
        ...
        for file_path in txt_files:
            if file_path.name != "extraction_summary.md":
                try:
                    ...
                    print(
                        f"✅ Analyzed: {api_data['api_name']} ({api_data['total_endpoints']} endpoints)"
                    )
    ...
    def generate_summary_report(self) -> str:
        ...
        report.append(
            f"📈 Total Endpoints Found: {sum(api['total_endpoints'] for api in self.results.values())}"
        )
        report.append("")
    ...
    def categorize_endpoints(self, endpoints: Dict) -> Dict[str, List[str]]:
        ...
        for endpoint in endpoints:
            if any(word in endpoint.lower() for word in ["team", "teams"]):
                ...
                for word in ["fixture", "match", "game", "fight"]
            ):
                ...
                word in endpoint.lower() for word in ["league", "competition", "season"]
            ):
                categories["Leagues/Competitions"].append(endpoint)
            elif any(
                word in endpoint.lower() for word in ["timezone", "country", "search"]
            ):
    ...
    def find_common_endpoints(self) -> Dict[str, List[str]]:
        ...
        return {
            endpoint: apis for endpoint, apis in endpoint_apis.items() if len(apis) > 1
        }
...
def main():
    ...
    print(
        f"🎯 Total Endpoints: {sum(api['total_endpoints'] for api in results.values())}"
    )
    ...
    sorted_apis = sorted(
        results.items(), key=lambda x: x[1]["total_endpoints"], reverse=True
    )
    print(
        f"🏆 Most Comprehensive API: {sorted_apis[0][0]} ({sorted_apis[0][1]['total_endpoints']} endpoints)"
    )
    ...
    if common_endpoints:
        ...
        print(
            f"🔄 Most Common Endpoint: '{most_common[0]}' (used in {len(most_common[1])} APIs)"
        )
