...
async def run_platinum_migration():
    ...
    try:
        ...
        migration_file = (
            ...
            / "bot"
            / "migrations"
            / "005_platinum_channel_support.sql"
        )
        ...
        for i, statement in enumerate(statements, 1):
            if statement:
                try:
                    ...
                    logger.warning(
                        f"⚠️ Statement {i} failed (this might be expected): {e}"
                    )
...
async def verify_platinum_migration(db_manager):
    ...
    try:
        result = await db_manager.fetch_one("SHOW TABLES LIKE 'platinum_features'")
        if result:
    ...
    try:
        result = await db_manager.fetch_one("SHOW TABLES LIKE 'webhook_integrations'")
        if result:
    ...
    try:
        result = await db_manager.fetch_one("SHOW TABLES LIKE 'data_exports'")
        if result:
    ...
    try:
        result = await db_manager.fetch_one("SHOW TABLES LIKE 'platinum_analytics'")
        if result:
    ...
    try:
        ...
        for guild in guilds:
            logger.info(
                f"   Guild {guild['guild_id']}: {guild['subscription_level']} (Platinum: {guild['platinum_features_enabled']})"
            )
    except Exception as e:
