...
def summarize_csv(filename):
    ...
    sorted_leagues = sorted(league_team_counts.items(), key=lambda x: x[1], reverse=True)
    for league, count in sorted_leagues[:10]:
        print(f"  {league}: {count} teams")
    
    print()
    print("Leagues with most players:")
    sorted_player_leagues = sorted(league_player_counts.items(), key=lambda x: x[1], reverse=True)
