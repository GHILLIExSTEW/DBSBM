#!/usr/bin/env python3
"""
Real ML Commands - Discord Commands with Live API Integration

These commands use real sports data from API-Sports to provide actionable
betting insights powered by ML models.
"""
...
class RealMLCommands(commands.Cog):
    """Real ML commands that use live API data."""
    ...
    @app_commands.command(
        name="analyze_today",
        description="Get today's best betting opportunities using real sports data",
    )
    async def analyze_today(self, interaction: discord.Interaction):
        ...
        try:
            ...
            real_ml_service = RealMLService(self.bot.db_manager, self.bot.sports_api)
    ...
    @app_commands.command(
        name="analyze_game",
        description="Analyze a specific game using real sports data",
    )
    ...
    async def analyze_game(
        ...
        """Analyze a specific game using real API data and ML."""
    ...
    @app_commands.command(
        name="my_insights",
        description="Get personalized betting insights based on your history",
    )
    ...
    @app_commands.command(
        name="live_odds", description="Get live odds for today's games"
    )
    async def live_odds(self, interaction: discord.Interaction):
        """Get live odds for today's major games."""
    ...
    @app_commands.command(
        name="team_stats", description="Get team statistics and recent performance"
    )
    @app_commands.describe(
    ...
    async def team_stats(
        ...
        """Get team statistics and recent performance."""
    ...
    async def _get_todays_games_with_odds(self, date: str) -> List[Dict]:
        ...
        try:
            ...
            # Get today's games
            games = await real_ml_service._get_todays_games_from_api(date)
            ...
            for game in games[:10]:  # Limit to 10 games
                try:
                    ...
                    odds = await real_ml_service._get_game_odds(
    ...
    async def _get_team_statistics(self, team_name: str, sport: str) -> Optional[Dict]:
        ...
        try:
            ...
            team_data = await real_ml_service._find_team_in_api(team_name, sport)

            if not team_data:
                return None

            # Get detailed team stats
            team_stats = await real_ml_service._get_team_stats(sport, team_data["id"])
