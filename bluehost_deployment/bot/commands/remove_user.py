...
class RemoveUserSelect(Select):
    ...
    def __init__(self, users: list, parent_view):
        ...
        options = [
            SelectOption(
                label=(
                    user["display_name"][:100]
                    if user.get("display_name")
                    else f"User ID: {user['user_id']}"
                ),
    ...
    async def callback(self, interaction: Interaction):
        ...
        if not db_manager:
            ...
            await interaction.response.send_message(
                "❌ Internal configuration error.", ephemeral=True
            )
            return
        ...
        try:
            ...
            if delete_status and "DELETE 1" in delete_status:
                logger.info(
                    f"User {user_id_to_remove} removed from users table for guild {interaction.guild_id} by {interaction.user}"
                )
                ...
                logger.warning(
                    f"Attempted to remove user {user_id_to_remove} for guild {interaction.guild_id}, but they were not found in the users table."
                )
                await interaction.followup.send(
                ...
                logger.error(
                    f"Failed to remove user {user_id_to_remove} for guild {interaction.guild_id}. Status: {delete_status}"
                )
                await interaction.followup.send(
                    "❌ Failed to remove user from database.", ephemeral=True
                )
...
class RemoveUserView(View):
            ...
            if not users:
                ...
                if not self.original_interaction.response.is_done():
                    ...
                    await self.original_interaction.followup.send(
                        "❌ No registered users found in this server to remove.",
                        ephemeral=True,
                    )
    ...
    async def on_timeout(self) -> None:
        logger.info(
            f"RemoveUserView timed out for interaction {self.original_interaction.id}"
        )
        if self.select_menu:
            self.select_menu.disabled = True
        try:
            await self.original_interaction.edit_original_response(
                content="Timed out waiting for user selection.", view=self
            )
...
class RemoveUserCog(commands.Cog):
    ...
    @app_commands.command(
        name="remove_user",
        description="Remove a registered user from the system (Admin only).",
    )
    ...
    async def remove_user_command(self, interaction: Interaction):
        ...
        logger.info(
            f"Remove User command initiated by {interaction.user} in guild {interaction.guild_id}"
        )
        try:
            ...
            if not interaction.response.is_done():
                await interaction.response.send_message(
                    "❌ An error occurred starting the remove user command.",
                    ephemeral=True,
                )
