...
from graphql import (
    GraphQLSchema, GraphQLObjectType, GraphQLField, GraphQLString, GraphQLInt,
    GraphQLFloat, GraphQLBoolean, GraphQLList, GraphQLNonNull, GraphQLEnumType,
    GraphQLInputObjectType, GraphQLArgument, GraphQLScalarType, graphql, parse, validate
)
...
class GraphQLService:
    ...
    async def publish_event(self, event_type: str, event_data: Dict[str, Any],
        ...
        try:
            ...
            for sub_id, subscription in self.subscriptions.items():
                if await self._subscription_matches_event(subscription, event_type, event_data, filters):
    ...
    async def get_schema(self, api_version: str = "v1") -> str:
        ...
        try:
            ...
            result = await self.execute_query(introspection_query, api_version=api_version)
            ...
            while True:
                try:
                    ...
                    for sub_id, subscription in self.subscriptions.items():
                        ...
                        if (current_time - subscription.last_activity) > timedelta(hours=1):
    ...
    async def _check_rate_limit(self, user_id: Optional[int], tenant_id: Optional[int],
        ...
        try:
            ...
            if usage_data:
                ...
                usage = {
                    "queries": {"count": 0, "reset_time": (current_time + timedelta(minutes=1)).isoformat()},
                    "mutations": {"count": 0, "reset_time": (current_time + timedelta(minutes=1)).isoformat()}
                }
        ...
        try:
            ...
            if filters:
                for key, value in filters.items():
                    if key in subscription.variables and subscription.variables[key] != value:
                        return False
    ...
    async def _resolve_users(self, info: GraphQLResolveInfo, limit: int = 10, offset: int = 0):
        """Resolve users list."""
        try:
            context = info.context
            db_manager = context.get('db_manager')
    ...
    async def _resolve_user_bets(self, info: GraphQLResolveInfo, limit: int = 10, offset: int = 0):
        """Resolve user's bets."""
        try:
            context = info.context
            db_manager = context.get('db_manager')
