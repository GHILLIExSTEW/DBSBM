...
class RealMLService:
    ...
    def __init__(self, db_manager: DatabaseManager, sports_api: SportsAPI, predictive_service=None):
    ...
    async def analyze_specific_game(self, home_team: str, away_team: str, sport: str, user_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            if not user_bets:
                return {
                    ...
                    'recommendations': ['Try analyzing today\'s games with /analyze_today']
                }
            ...
            recommendations = await self._generate_personalized_recommendations(patterns, user_id)
        ...
        for sport in sports:
            try:
                ...
                for league in leagues[:3]:  # Top 3 leagues per sport
                    league_games = await self._get_league_games(sport, league['id'], date)
    ...
    async def _get_league_games(self, sport: str, league_id: int, date: str) -> List[Dict]:
    ...
    async def _find_game_in_api(self, home_team: str, away_team: str, sport: str) -> Optional[Dict]:
    ...
    async def _get_game_details(self, game_id: int, sport: str) -> Optional[Dict]:
        ...
        try:
            async with self.fetcher:
                # Get fixture details
                fixture_data = await self.fetcher.fetch_data(sport, "fixtures", {'id': game_id})
    ...
    async def _get_team_stats(self, sport: str, team_id: int) -> Dict:
        ...
        try:
            async with self.fetcher:
                # Get team standings
                standings_data = await self.fetcher.fetch_data(sport, "standings", {'team': team_id})
    ...
    async def _get_game_odds(self, fixture_id: int) -> Dict:
        ...
        try:
            ...
            async with self.fetcher:
                odds_data = await self.fetcher.fetch_data("football", "odds", {'fixture': fixture_id})
    ...
    async def _prepare_ml_input(self, game: Dict, odds: Dict) -> Dict:
        ...
        try:
            ...
            input_data = {
                ...
                'venue': 'home',
                'league_importance': game.get('league', {}).get('country', {}).get('name', 'Unknown')
            }
    ...
    async def _make_ml_prediction(self, input_data: Dict, user_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            if prediction:
                ...
                reasoning = await self._generate_reasoning(input_data, prediction.prediction_result)
    ...
    async def _calculate_optimal_bet_size(self, user_id: int, prediction: Dict, odds: Dict) -> Dict:
        ...
        try:
            ...
            kelly_fraction = (win_probability * odds['home_win'] - 1) / (odds['home_win'] - 1)
    ...
    async def _analyze_betting_patterns(self, user_bets: List[Dict]) -> Dict:
        ...
        preferred_sports = sorted(sports_count.items(), key=lambda x: x[1], reverse=True)[:3]
        ...
        return {
            ...
            'risk_profile': 'high' if avg_bet_size > 100 else 'medium' if avg_bet_size > 50 else 'low'
        }

    async def _generate_personalized_recommendations(self, patterns: Dict, user_id: int) -> List[str]:
        """Generate personalized recommendations based on betting patterns."""

        recommendations = []

        if patterns.get('success_rate', 0) < 0.5:
            recommendations.append("Consider reducing bet sizes and focusing on safer bets")

        if patterns.get('avg_bet_size', 0) > 100:
            recommendations.append("Your bet sizes are high - consider bankroll management")
        ...
        if preferred_sports:
            recommendations.append(f"Check today's {preferred_sports[0]} games for opportunities")
