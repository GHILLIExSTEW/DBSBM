...
class APISportsFetcher:
    ...
    async def fetch_data(self, sport: str, endpoint: str, params: Dict[str, Any]) -> Dict:
        ...
        try:
            if sport == 'football':
                ...
                return {
                    ...
                    'sport': sport.title(),
                    'league': LEAGUE_CONFIG.get(sport, {}).get(league, {}).get('name', league),
                    ...
                    'status': game.get('status', {}).get('long', game.get('fixture', {}).get('status', {}).get('long', 'Unknown')),
                    'score': {
                        'home': game.get('scores', {}).get('home', {}).get('total', game.get('goals', {}).get('home', 0)),
                        'away': game.get('scores', {}).get('away', {}).get('total', game.get('goals', {}).get('away', 0))
                    } if any(key in game for key in ['scores', 'goals']) else None,
                    'venue': game.get('venue', {}).get('name')
                }
...
class SportsAPI:
    def __init__(self, db_manager=None):
        ...
        self.split_season = self.current_year - 1 if datetime.now().month < 7 else self.current_year
            ...
            for sport, leagues in LEAGUE_CONFIG.items():
                for league in leagues:
                    try:
                        ...
                        games = await api.fetch_games(sport, league, date=current_date_str, end_date=tomorrow_date_str)
                        
                        if games:
                            # Save raw JSON
                            safe_league = league.replace("/", "_")
                            file_path = os.path.join(raw_data_dir, f"{current_date_str}_{sport}_{safe_league}.json")
                            ...
                            logger.info(f"Saved {len(games)} games for {sport}/{league} to {file_path}")
                        else:
