...
class ApiService:
    ...
    async def get_league_standings(self, league_key: str, season: Optional[int] = None) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Retrieved {len(standings)} standings entries for {league_key}")
            return standings
    ...
    async def get_team_logo(self, team_name: str, league_key: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Error fetching team logo for {team_name} in {league_key}: {str(e)}")
            return None
    ...
    def _get_sport_and_league_id(self, league_key: str) -> tuple[Optional[str], Optional[str]]:
