...
class VoiceService:
        ...
        while self.running:
            try:
                ...
                if not guilds_to_update:
                    logger.info("[VoiceService] No guilds found needing unit channel updates.")
    ...
    async def _update_guild_unit_channels(self, guild_info: Dict):
        ...
        try:
            ...
            logger.info(f"[VoiceService] Guild {guild_id} totals - Monthly: {monthly_total}, Yearly: {yearly_total}")
            ...
            if update_tasks:
                ...
                logger.info(f"[VoiceService] Channel updates completed for guild {guild_id}")
    ...
    async def _update_channel_name(self, channel_id: Optional[int], new_name: str):
        ...
        try:
            ...
            if not channel:
                logger.info(f"[VoiceService] Channel {channel_id} not in cache, fetching...")
                try:
                    channel = await self.bot.fetch_channel(channel_id)
                except discord.NotFound:
                    logger.warning(f"[VoiceService] Channel ID {channel_id} not found via fetch. Cannot update name.")
                    return
                except discord.Forbidden:
                    logger.error(f"[VoiceService] Permission error fetching channel {channel_id}. Bot needs 'View Channel'.")
                    return
                except Exception as fetch_err:
                    logger.error(f"[VoiceService] Error fetching channel {channel_id}: {fetch_err}")
                    return
            ...
            if isinstance(channel, discord.VoiceChannel):
                ...
                if channel.name != trimmed_name:
                    ...
                    logger.info(f"[VoiceService] Updated channel {channel_id} name to '{trimmed_name}'")
                else:
                    logger.info(f"[VoiceService] Channel {channel_id} name already up-to-date ('{channel.name}'). Skipping edit.")
            elif channel:
                logger.warning(f"[VoiceService] Channel ID {channel_id} is not a voice channel (type: {channel.type}). Cannot update name.")

        except discord.RateLimited as rl:
            retry_after = getattr(rl, 'retry_after', 5.0)
            logger.warning(f"[VoiceService] Rate limited updating channel {channel_id}. Discord asks to retry after {retry_after:.2f}s")
        except discord.errors.NotFound:
            logger.warning(f"[VoiceService] Channel {channel_id} not found during edit attempt (possibly deleted just now).")
        except discord.errors.Forbidden:
            logger.error(f"[VoiceService] Permission error updating channel {channel_id} name. Bot needs 'Manage Channels' permission.")
        except Exception as e:
            logger.exception(f"[VoiceService] Unexpected error updating channel name for {channel_id}: {e}")
