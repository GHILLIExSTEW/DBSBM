TENNIS_ABBREVIATIONS = {
    # ATP Tour Masters 1000
    "ind": "Indian Wells Masters",
    "miami": "Miami Open",
    "monte": "Monte-Carlo Masters",
    "madrid": "Madrid Open",
    "rome": "Italian Open",
    "canada": "Canadian Open",
    "cinci": "Cincinnati Masters",
    "shang": "Shanghai Masters",
    "paris": "Paris Masters",
    
    # ATP 500 Series
    "rotterdam": "Rotterdam Open",
    "rio": "Rio Open",
    "dubai": "Dubai Tennis Championships",
    "acapulco": "Mexican Open",
    "barcelona": "Barcelona Open",
    "hamburg": "Hamburg European Open",
    "washington": "Citi Open",
    "beijing": "China Open",
    "tokyo": "Japan Open",
    "vienna": "Vienna Open",
    "basel": "Swiss Indoors",
    
    # WTA 1000
    "doha": "Qatar Open",
    "dubai": "Dubai Tennis Championships",
    "indian": "Indian Wells Open",
    "miami": "Miami Open",
    "madrid": "Madrid Open",
    "rome": "Italian Open",
    "canada": "Canadian Open",
    "cincinnati": "Cincinnati Open",
    "wuhan": "Wuhan Open",
    "beijing": "China Open",
    
    # WTA 500
    "adelaide": "Adelaide International",
    "sydney": "Sydney International",
    "stuttgart": "Stuttgart Open",
    "berlin": "Berlin Open",
    "eastbourne": "Eastbourne International",
    "san jose": "Silicon Valley Classic",
    "tokyo": "Japan Open",
    "moscow": "Kremlin Cup",
    
    # ITF Tournaments
    "aus": "Australian Open",
    "french": "French Open",
    "wimbledon": "Wimbledon",
    "us": "US Open",
    "davis": "Davis Cup",
    "fed": "Fed Cup",
    "hopman": "Hopman Cup"
}

TOURNAMENT_FULL_NAMES = {
    # ATP Tour Masters 1000
    "ind": "Indian Wells Masters",
    "indian wells": "Indian Wells Masters",
    "bnp paribas": "Indian Wells Masters",
    
    "miami": "Miami Open",
    "miami masters": "Miami Open",
    
    "monte": "Monte-Carlo Masters",
    "monte carlo": "Monte-Carlo Masters",
    "monaco": "Monte-Carlo Masters",
    
    "madrid": "Madrid Open",
    "mutua madrid": "Madrid Open",
    
    "rome": "Italian Open",
    "italian": "Italian Open",
    "rome masters": "Italian Open",
    
    "canada": "Canadian Open",
    "rogers cup": "Canadian Open",
    "montreal": "Canadian Open",
    "toronto": "Canadian Open",
    
    "cinci": "Cincinnati Masters",
    "cincinnati": "Cincinnati Masters",
    "western & southern": "Cincinnati Masters",
    
    "shang": "Shanghai Masters",
    "shanghai": "Shanghai Masters",
    
    "paris": "Paris Masters",
    "bercy": "Paris Masters",
    "paris masters": "Paris Masters",
    
    # ATP 500 Series
    "rotterdam": "Rotterdam Open",
    "ahoy": "Rotterdam Open",
    
    "rio": "Rio Open",
    "rio de janeiro": "Rio Open",
    
    "dubai": "Dubai Tennis Championships",
    "dubai duty free": "Dubai Tennis Championships",
    
    "acapulco": "Mexican Open",
    "mexican": "Mexican Open",
    "abierto mexicano": "Mexican Open",
    
    "barcelona": "Barcelona Open",
    "barcelona open banc sabadell": "Barcelona Open",
    
    "hamburg": "Hamburg European Open",
    "german open": "Hamburg European Open",
    
    "washington": "Citi Open",
    "citi": "Citi Open",
    
    "beijing": "China Open",
    "china": "China Open",
    
    "tokyo": "Japan Open",
    "rakuten": "Japan Open",
    
    "vienna": "Vienna Open",
    "erst bank": "Vienna Open",
    
    "basel": "Swiss Indoors",
    "swiss": "Swiss Indoors",
    
    # WTA 1000
    "doha": "Qatar Open",
    "qatar": "Qatar Open",
    "qatar total": "Qatar Open",
    
    "indian": "Indian Wells Open",
    "indian wells": "Indian Wells Open",
    "bnp paribas": "Indian Wells Open",
    
    "wuhan": "Wuhan Open",
    "dongfeng": "Wuhan Open",
    
    # WTA 500
    "adelaide": "Adelaide International",
    "adelaide international": "Adelaide International",
    
    "sydney": "Sydney International",
    "sydney international": "Sydney International",
    
    "stuttgart": "Stuttgart Open",
    "porsche tennis": "Stuttgart Open",
    
    "berlin": "Berlin Open",
    "german open": "Berlin Open",
    
    "eastbourne": "Eastbourne International",
    "eastbourne international": "Eastbourne International",
    
    "san jose": "Silicon Valley Classic",
    "silicon valley": "Silicon Valley Classic",
    "mubadala": "Silicon Valley Classic",
    
    "moscow": "Kremlin Cup",
    "kremlin": "Kremlin Cup",
    
    # ITF Tournaments
    "aus": "Australian Open",
    "australian": "Australian Open",
    "ao": "Australian Open",
    
    "french": "French Open",
    "roland garros": "French Open",
    "rg": "French Open",
    
    "wimbledon": "Wimbledon",
    "wimby": "Wimbledon",
    
    "us": "US Open",
    "us open": "US Open",
    "uso": "US Open",
    
    "davis": "Davis Cup",
    "davis cup": "Davis Cup",
    
    "fed": "Fed Cup",
    "fed cup": "Fed Cup",
    "billie jean king cup": "Fed Cup",
    
    "hopman": "Hopman Cup",
    "hopman cup": "Hopman Cup"
}

PLAYER_FULL_NAMES = {
    # ATP Players - Top 50+
    "djokovic": "Novak Djokovic",
    "nadal": "Rafael Nadal",
    "federer": "Roger Federer",
    "medvedev": "Daniil Medvedev",
    "tsitsipas": "Stefanos Tsitsipas",
    "zverev": "Alexander Zverev",
    "alcaraz": "Carlos Alcaraz",
    "sinner": "Jannik Sinner",
    "rublev": "Andrey Rublev",
    "ruud": "Casper Ruud",
    "fritz": "Taylor Fritz",
    "tiafoe": "Frances Tiafoe",
    "hurkacz": "Hubert Hurkacz",
    "rune": "Holger Rune",
    "norrie": "Cameron Norrie",
    "auger": "Felix Auger-Aliassime",
    "shapovalov": "Denis Shapovalov",
    "berrettini": "Matteo Berrettini",
    "korda": "Sebastian Korda",
    "de minaur": "Alex de Minaur",
    "musetti": "Lorenzo Musetti",
    "coric": "Borna Coric",
    "cerundolo": "Francisco Cerundolo",
    "dimitrov": "Grigor Dimitrov",
    "khachanov": "Karen Khachanov",
    "bautista": "Roberto Bautista Agut",
    "carreno": "Pablo Carreno Busta",
    "schwartzman": "Diego Schwartzman",
    "isner": "John Isner",
    "monfils": "Gael Monfils",
    "wawrinka": "Stan Wawrinka",
    "thiem": "Dominic Thiem",
    "nishikori": "Kei Nishikori",
    "del potro": "Juan Martin del Potro",
    "murray": "Andy Murray",
    "cilic": "Marin Cilic",
    "gasquet": "Richard Gasquet",
    "simon": "Gilles Simon",
    "tsonga": "Jo-Wilfried Tsonga",
    "verdasco": "Fernando Verdasco",
    "lopez": "Feliciano Lopez",
    "roddick": "Andy Roddick",
    "hewitt": "Lleyton Hewitt",
    "safin": "Marat Safin",
    "agassi": "Andre Agassi",
    "sampras": "Pete Sampras",
    "courier": "Jim Courier",
    "edberg": "Stefan Edberg",
    "becker": "Boris Becker",
    "lendl": "Ivan Lendl",
    
    # WTA Players - Top 50+
    "swiatek": "Iga Swiatek",
    "sabalenka": "Aryna Sabalenka",
    "rybakina": "Elena Rybakina",
    "gauff": "Coco Gauff",
    "jabeur": "Ons Jabeur",
    "pegula": "Jessica Pegula",
    "sakkari": "Maria Sakkari",
    "garcia": "Caroline Garcia",
    "kvitova": "Petra Kvitova",
    "muguruza": "Garbine Muguruza",
    "kudermetova": "Veronika Kudermetova",
    "kasatkina": "Daria Kasatkina",
    "bencic": "Belinda Bencic",
    "samsonova": "Liudmila Samsonova",
    "ostapenko": "Jelena Ostapenko",
    "pliskova": "Karolina Pliskova",
    "svitolina": "Elina Svitolina",
    "halep": "Simona Halep",
    "kerber": "Angelique Kerber",
    "wozniacki": "Caroline Wozniacki",
    "azarenka": "Victoria Azarenka",
    "kvitova": "Petra Kvitova",
    "sharapova": "Maria Sharapova",
    "williams": "Serena Williams",
    "venus": "Venus Williams",
    "davenport": "Lindsay Davenport",
    "hingis": "Martina Hingis",
    "graf": "Steffi Graf",
    "seles": "Monica Seles",
    "navratilova": "Martina Navratilova",
    "evert": "Chris Evert",
    "king": "Billie Jean King",
    "court": "Margaret Court",
    "goolagong": "Evonne Goolagong",
    "austin": "Tracy Austin",
    "capriati": "Jennifer Capriati",
    "sanchez": "Arantxa Sanchez Vicario",
    "sabatini": "Gabriela Sabatini",
    "novotna": "Jana Novotna",
    "coetzer": "Amanda Coetzer",
    "mauresmo": "Amelie Mauresmo",
    "clijsters": "Kim Clijsters",
    "henin": "Justine Henin",
    "dementieva": "Elena Dementieva",
    "kuznetsova": "Svetlana Kuznetsova",
    "myskina": "Anastasia Myskina",
    "schiavone": "Francesca Schiavone",
    "ivanovic": "Ana Ivanovic",
    "jankovic": "Jelena Jankovic",
    "safina": "Dinara Safina",
    "zvonareva": "Vera Zvonareva",
    "pennetta": "Flavia Pennetta",
    "stosur": "Samantha Stosur",
    "bartoli": "Marion Bartoli",
    "radwanska": "Agnieszka Radwanska",
    "li": "Li Na",
    "kvitova": "Petra Kvitova",
    "schiavone": "Francesca Schiavone",
    "stosur": "Samantha Stosur",
    "bartoli": "Marion Bartoli",
    "radwanska": "Agnieszka Radwanska",
    "li": "Li Na",
    "kvitova": "Petra Kvitova",
    "schiavone": "Francesca Schiavone",
    "stosur": "Samantha Stosur",
    "bartoli": "Marion Bartoli",
    "radwanska": "Agnieszka Radwanska",
    "li": "Li Na"
} 