...
from config.prop_templates import get_prop_templates_for_league, format_prop_line
...
class EnhancedPlayerPropImageGenerator:
    ...
    async def generate_player_prop_image(
        ...
        try:
            ...
            await self._add_images(draw, player_image_url, team_logo_url, team_name, league)
            
            # Add main content
            self._add_player_info(draw, player_name, team_name, league)
            self._add_prop_details(draw, prop_type, line_value, bet_direction, bet_amount, odds)
    ...
    def _draw_card_background(self, draw: ImageDraw.Draw):
        ...
        draw.rounded_rectangle(border_rect, radius=17, outline=self.colors['border'], width=2)
    
    async def _add_images(self, draw: ImageDraw.Draw, player_image_url: str, team_logo_url: str, team_name: str, league: str):
        ...
        try:
            ...
            if team_logo:
                ...
                img.paste(team_logo, (40, 40), team_logo if team_logo.mode == 'RGBA' else None)
            ...
            if player_image_url:
                ...
                if player_image:
                    ...
                    player_image = player_image.resize((120, 120), Image.Resampling.LANCZOS)
                    # Apply circular mask
    ...
    def _add_player_info(self, draw: ImageDraw.Draw, player_name: str, team_name: str, league: str):
    ...
    def _add_prop_details(self, draw: ImageDraw.Draw, prop_type: str, line_value: float, bet_direction: str, bet_amount: float, odds: Optional[float]):
        ...
        direction_color = self.colors['over_green'] if bet_direction == 'over' else self.colors['under_red']
    ...
    def _add_performance_stats(self, draw: ImageDraw.Draw, stats: Dict[str, Any], prop_type: str):
    ...
    def _add_decorative_elements(self, draw: ImageDraw.Draw, prop_type: str, bet_direction: str):
    ...
    def _add_comparison_chart(self, draw: ImageDraw.Draw, recent_games: List[Dict[str, Any]], line_value: float, prop_type: str):
        ...
        try:
            ...
            chart_rect = [chart_x, chart_y, chart_x + chart_width, chart_y + chart_height]
            draw.rounded_rectangle(chart_rect, radius=10, fill=self.colors['background'])
            
            # Draw line value reference
            line_y = chart_y + chart_height - 50
            draw.line([chart_x, line_y, chart_x + chart_width, line_y], fill=self.colors['border'], width=2)
            draw.text(
            ...
            if recent_games:
                ...
                max_value = max([game.get('value', 0) for game in recent_games] + [line_value])
                ...
                for i, game in enumerate(recent_games[:10]):  # Limit to 10 games
                    ...
                    bar_color = self.colors['over_green'] if value > line_value else self.colors['under_red']
                    
                    draw.rectangle([bar_x, bar_y, bar_x + bar_width - 20, chart_y + chart_height - 50], fill=bar_color)
