...
def format_units_display(
    units: float, display_as_risk: bool, unit_label: str = None
) -> str:
    """
...
def calculate_parlay_payout(legs, units, odds_type="american"):
    ...
    if not legs or not units:
        return {"total_odds": 0, "payout": 0, "profit": 0, "risk": units}
    ...
    try:
        ...
        for leg in legs:
            ...
            try:
                ...
                logger.debug(
                    f"Converted American odds {odds_val} to decimal {decimal_odds}"
                )
        ...
        logger.info(
            f"Parlay calculation: {len(legs)} legs, {units} units, "
