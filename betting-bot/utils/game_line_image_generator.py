...
class GameLineImageGenerator:
    ...
    def draw_teams_section(self, img, draw, image_width, home_team, away_team, home_logo, away_logo, selected_team):
        ...
        vs_text = "VS"
        vs_w, vs_h = draw.textsize(vs_text, font=vs_font)
        ...
        home_name_w, home_name_h = draw.textsize(home_team, font=team_name_font)
        ...
        away_name_w, away_name_h = draw.textsize(away_team, font=team_name_font)
    ...
    def draw_straight_details(self, draw, image_width, image_height, line, odds, units, bet_id, timestamp, img, start_y):
        ...
        if line:
            line = line.upper()
            line_w, line_h = draw.textsize(line, font=line_font)
        ...
        odds_w, odds_h = draw.textsize(odds_text, font=odds_font)
        draw.text((center_x, y), odds_text, font=odds_font, fill=text_color, anchor="mt")
        y += odds_h + 12

        # Stake / To Risk logic
        payout_text = f"To Risk {units:.2f} Units"
        payout_text_w, payout_text_h = draw.textsize(payout_text, font=units_font)
    ...
    def draw_footer(self, draw, image_width, image_height, bet_id, timestamp):
        ...
        footer_w, footer_h = draw.textsize(footer_text, font=footer_font)
