...
try:
    ...
    print(f"CRITICAL ERROR: Could not import from config package: {e}. "
          ...
          f"BASE_DIR was: {BASE_DIR}", file=sys.stderr)
...
SAVE_BASE_PATH = os.path.join(STATIC_DIR, "logos", "teams") # This will be static/logos/teams/
...
def main():
    ...
    with open(CSV_FILE_PATH, 'r', newline='', encoding='utf-8') as csvfile:
        ...
        if not reader.fieldnames or not all(f in reader.fieldnames for f in ['league', 'team_name', 'logo_url']):
            logger.error("CSV file is missing required columns: 'league', 'team_name', 'logo_url'")
            return
