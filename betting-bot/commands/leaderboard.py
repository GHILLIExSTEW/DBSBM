...
class LeaderboardCog(commands.Cog):
    ...
    async def leaderboard(self, interaction: Interaction, metric: str = "net_units", limit: int = 10):
        ...
        try:
            ...
            if not cappers:
                await interaction.followup.send("❌ No capper data available for leaderboard.", ephemeral=True)
                return
            ...
            title = f"🏆 Top {limit} Cappers by {metric_names.get(metric, metric.title())}"
    ...
    @app_commands.command(
        ...
        description="Compare stats between multiple cappers"
    )
    async def stats_comparison(self, interaction: Interaction):
        ...
        try:
            ...
            if not cappers:
                await interaction.followup.send("❌ No cappers found in this guild.", ephemeral=True)
                return
            ...
            if not top_cappers:
                await interaction.followup.send("❌ No capper data available for comparison.", ephemeral=True)
                return
            ...
            await interaction.followup.send(
                content="📊 Top 5 Cappers Comparison",
                file=file,
                ephemeral=True
            )
