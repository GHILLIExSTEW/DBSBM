...
class MaintenanceCog(commands.Cog):
    ...
    async def down_command(self, interaction: Interaction):
        ...
        if interaction.user.id != AUTHORIZED_USER_ID:
            await interaction.response.send_message("❌ Unauthorized. Only authorized users can use this command.", ephemeral=True)
            return
                ...
                for channel_id in command_channels:
                    try:
                        ...
                        if channel:
                            ...
                            sent_count += 1
                            logger.info(f"Sent down notification to guild {guild_id}, channel {channel_id}")
                        else:
                            logger.error(f"Could not find channel {channel_id} for guild {guild_id}")
                            failed_count += 1
                    except Exception as e:
                        logger.error(f"Failed to send down notification to guild {guild_id}, channel {channel_id}: {e}")
                        failed_count += 1
                ...
                for channel_id in command_channels:
                    try:
                        ...
                        if channel:
                            await channel.send(embed=embed)
                            sent_count += 1
                            logger.info(f"Sent up notification to guild {guild_id}, channel {channel_id}")
                        else:
                            logger.error(f"Could not find channel {channel_id} for guild {guild_id}")
                            failed_count += 1
                    except Exception as e:
                        logger.error(f"Failed to send up notification to guild {guild_id}, channel {channel_id}: {e}")
                        failed_count += 1
