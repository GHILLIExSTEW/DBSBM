...
class AddUserCog(commands.Cog):
    ...
    @app_commands.command(
        name="add_user", description="Add a user as a capper (admin only)"
    )
    ...
    async def add_user_command(self, interaction: Interaction, user: Member):
        ...
        try:
            ...
            logger.debug(
                f"Executing INSERT INTO cappers for guild_id={guild_id}, user_id={user_id}"
            )
            ...
            logger.info(
                f"Successfully executed INSERT for user {user_id} in guild {guild_id}"
            )
            # Check if paid guild
...
async def setup(bot: commands.Bot):
    ...
    try:
        if TEST_GUILD_ID:
            ...
            logger.info(
                f"Successfully synced add_user command to test guild {TEST_GUILD_ID}"
            )
        else:
            logger.warning(
