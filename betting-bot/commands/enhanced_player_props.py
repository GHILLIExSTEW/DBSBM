...
class EnhancedPlayerPropsCog(commands.Cog):
    ...
    async def playerprops(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed.set_footer(text="Click a button below to select a league and start creating player prop bets")
...
class LeagueSelectionView(discord.ui.View):
    ...
    def create_league_callback(self, league_key: str):
        ...
        async def callback(interaction: discord.Interaction):
            try:
                ...
                logger.error(f"Error in league callback for {league_key}: {e}", exc_info=True)
                await interaction.response.send_message(
                    f"❌ **Error:** {str(e)}",
                    ephemeral=True
                )
    ...
    async def cancel_callback(self, interaction: discord.Interaction):
        ...
        try:
            await interaction.response.edit_message(
                content="Player props cancelled.",
                view=None,
                embed=None
            )
        except Exception as e:
    ...
    async def handle_league_selection(self, interaction: discord.Interaction, league_key: str):
        ...
        try:
            ...
            for game in games:
                formatted_games.append({
                    ...
                    'game_time': game['game_time'].strftime('%m/%d %I:%M %p') if game['game_time'] else 'TBD',
                    'league': game['league']
                })
            ...
            logger.error(f"Error getting available games for {league_key}: {e}", exc_info=True)
            return []
...
class TeamSelectionView(discord.ui.View):
    ...
    def __init__(self, bot: commands.Bot, db_manager, league_key: str, game_id: str, home_team: str, away_team: str):
        super().__init__(timeout=300)
        self.bot = bot
        self.db_manager = db_manager
        self.league_key = league_key
    ...
    async def back_callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            view = GameSelectionView(self.bot, self.db_manager, self.league_key, games[:10])
