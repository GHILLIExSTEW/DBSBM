...
async def migrate_cappers_images():
    db = DatabaseManager()
    await db.connect()
    cappers = await db.fetch_all("SELECT guild_id, user_id, image_path FROM cappers WHERE image_path IS NOT NULL AND image_path != ''")
            ...
            if old_path.startswith('http://') or old_path.startswith('https://'):
                ...
                try:
                    ...
                    await db.execute(
                        ...
                        new_url, guild_id, user_id
                    )
                    print(f"Updated DB for guild {guild_id}, user {user_id}")
                except Exception as e:
                    print(f"Error downloading image for guild {guild_id}, user {user_id}: {e}")
            else:
                # Local file path
