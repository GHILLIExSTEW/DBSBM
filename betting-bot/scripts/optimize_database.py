...
class DatabaseOptimizer:
    ...
    async def analyze_table_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for table in tables:
                ...
                recommendations = await self._generate_table_recommendations(table, table_stats)
    ...
    async def _generate_table_recommendations(self, table_name: str, stats: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if row_count > 1000000:  # 1 million rows
                recommendations.append(f"Consider partitioning table '{table_name}' (has {row_count:,} rows)")
            
            # Check for missing indexes
            if table_name == 'bets':
                if not any(idx['column_name'] == 'guild_id' for idx in indexes):
                    recommendations.append(f"Add index on bets.guild_id for better query performance")
                if not any(idx['column_name'] == 'user_id' for idx in indexes):
                    recommendations.append(f"Add index on bets.user_id for better query performance")
                if not any(idx['column_name'] == 'created_at' for idx in indexes):
                    recommendations.append(f"Add index on bets.created_at for better date range queries")
            
            elif table_name == 'cappers':
                if not any(idx['column_name'] == 'guild_id' for idx in indexes):
                    recommendations.append(f"Add index on cappers.guild_id for better query performance")
                if not any(idx['column_name'] == 'user_id' for idx in indexes):
                    recommendations.append(f"Add index on cappers.user_id for better query performance")
            
            elif table_name == 'games':
                if not any(idx['column_name'] == 'game_time' for idx in indexes):
                    recommendations.append(f"Add index on games.game_time for better date range queries")
                if not any(idx['column_name'] == 'status' for idx in indexes):
                    recommendations.append(f"Add index on games.status for better status filtering")
            
            # Check for large table size
            if size_mb > 100:  # 100 MB
                recommendations.append(f"Table '{table_name}' is large ({size_mb} MB). Consider archiving old data")
    ...
    async def cleanup_old_data(self, days_to_keep: int = 90) -> Dict[str, Any]:
        ...
        try:
            ...
            deleted_bets = await self.db_manager.execute(
                "DELETE FROM bets WHERE created_at < %s",
                (cutoff_date,)
            )
            results['deleted_bets'] = deleted_bets.get('affected_rows', 0) if deleted_bets else 0
            ...
            results['deleted_games'] = deleted_games.get('affected_rows', 0) if deleted_games else 0
            
            logger.info(f"Deleted {results['deleted_bets']} old bets and {results['deleted_games']} old games")
