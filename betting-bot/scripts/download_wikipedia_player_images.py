...
class WikipediaPlayerImageDownloader:
        ...
        self.player_lists = {
            'tennis': {
                'ATP': [
                    ...
                    'Felix Auger-Aliassime', 'Denis Shapovalov', 'Grigor Dimitrov', 'Lorenzo Musetti'
                ],
                'WTA': [
                    'Iga Świątek', 'Aryna Sabalenka', 'Coco Gauff', 'Elena Rybakina',
                    'Jessica Pegula', 'Ons Jabeur', 'Markéta Vondroušová', 'Karolína Muchová',
                    'Maria Sakkari', 'Barbora Krejčíková', 'Daria Kasatkina', 'Veronika Kudermetova',
                    'Madison Keys', 'Belinda Bencic', 'Liudmila Samsonova', 'Victoria Azarenka',
                    'Petra Kvitová', 'Caroline Garcia', 'Beatriz Haddad Maia', 'Elina Svitolina'
                ]
            },
            'darts': {
                'PDC': [
                    'Luke Humphries', 'Michael van Gerwen', 'Michael Smith', 'Nathan Aspinall',
                    'Gerwyn Price', 'Rob Cross', 'Peter Wright', 'Dimitri Van den Bergh',
                    'Jonny Clayton', 'Danny Noppert', 'Joe Cullen', 'Damon Heta',
                    'Dave Chisnall', 'Chris Dobey', 'Ross Smith', 'Josh Rock',
                    'Andrew Gilding', 'Stephen Bunting', 'Ryan Searle', 'Krzysztof Ratajski'
                ],
                'BDO': [
                    'Glen Durrant', 'Scott Waites', 'Scott Mitchell', 'Mark McGeeney',
                    'Jim Williams', 'Wayne Warren', 'Mikuru Suzuki', 'Lisa Ashton',
                    'Anastasia Dobromyslova', 'Deta Hedman', 'Fallon Sherrock', 'Beau Greaves'
                ]
            },
            'golf': {
                'PGA': [
                    'Scottie Scheffler', 'Rory McIlroy', 'Jon Rahm', 'Viktor Hovland',
                    'Patrick Cantlay', 'Xander Schauffele', 'Max Homa', 'Wyndham Clark',
                    'Brian Harman', 'Tommy Fleetwood', 'Tyrrell Hatton', 'Jordan Spieth',
                    'Justin Thomas', 'Collin Morikawa', 'Sam Burns', 'Tony Finau',
                    'Sungjae Im', 'Cameron Young', 'Keegan Bradley', 'Rickie Fowler'
                ],
                'LPGA': [
                    'Lilia Vu', 'Celine Boutier', 'Ruoning Yin', 'Atthaya Thitikul',
                    'Nelly Korda', 'Jin Young Ko', 'Minjee Lee', 'Hyo Joo Kim',
                    'Charley Hull', 'Linn Grant', 'Leona Maguire', 'Brooke Henderson',
                    'Danielle Kang', 'Nasa Hataoka', 'Hannah Green', 'Alison Lee',
                    'Megan Khang', 'Jennifer Kupcho', 'Andrea Lee', 'Grace Kim'
                ]
            }
        }
    ...
    def search_wikipedia_player(self, player_name):
        ...
        try:
            ...
            response = requests.get(self.wikipedia_search_url, params=search_params, timeout=10)
            if response.status_code == 200:
                data = response.json()
                    ...
                    if summary_response.status_code == 200:
                        ...
                        if 'thumbnail' in summary_data and summary_data['thumbnail']:
                            return {
                                ...
                                'page_url': summary_data['content_urls']['desktop']['page']
                            }
    ...
    def download_and_process_image(self, image_url, player_name, sport, league):
        ...
        try:
            ...
            with Image.open(BytesIO(response.content)) as img:
                ...
                # Create filename
                safe_name = player_name.lower().replace(' ', '_').replace('ć', 'c').replace('ś', 's').replace('ń', 'n').replace('ó', 'o').replace('ł', 'l').replace('ż', 'z').replace('ź', 'z')
    ...
    def print_final_summary(self, results):
        ...
        for sport, leagues in results.items():
            ...
            for league, league_results in leagues.items():
                ...
                logger.info(f"  {league}: {successful} successful, {failed} failed, {not_found} not found")
        
        logger.info(f"\nTOTALS:")
        logger.info(f"  Successful: {total_successful}")
        logger.info(f"  Failed: {total_failed}")
        logger.info(f"  Not Found: {total_not_found}")
        logger.info(f"  Total Attempted: {total_successful + total_failed + total_not_found}")
            ...
            for sport, leagues in results.items():
                for league, league_results in leagues.items():
                    if league_results['failed']:
                        logger.info(f"  {sport}/{league} - Failed: {', '.join(league_results['failed'])}")
                    if league_results['not_found']:
                        logger.info(f"  {sport}/{league} - Not Found: {', '.join(league_results['not_found'])}")
