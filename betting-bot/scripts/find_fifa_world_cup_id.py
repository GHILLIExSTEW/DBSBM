...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            BASE_DIR / "logs" / "find_fifa_world_cup_id.log", encoding="utf-8"
        ),
    ],
)
        ...
        try:
            ...
            async with self.session.get(url, params=params) as response:
                ...
                if "response" in data:
                    ...
                    logger.info(
                        f"Found {len(leagues)} leagues matching '{search_term}'"
                    )
            ...
            async with self.session.get(url, params=params) as response:
                ...
                if "response" in data:
                    ...
                    for i, team_data in enumerate(teams[:5]):
                        ...
                        logger.info(
                            f"Team {i+1}: {team.get('name')} ({team.get('country')}) - National: {team.get('national')}"
                        )
    ...
    async def test_potential_ids(self):
        ...
        for league_id in potential_ids:
            ...
            if teams:
                ...
                national_teams = [
                    t for t in teams if t.get("team", {}).get("national", False)
                ]
                if national_teams:
                    ...
                    for team in national_teams[:5]:
                        ...
                        logger.info(
                            f"  - {team_info.get('name')} ({team_info.get('country')})"
                        )
                    logger.info("-" * 50)
