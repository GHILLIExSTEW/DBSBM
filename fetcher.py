...
def fetch_games(league_key: str, date: Optional[str] = None) -> List[Dict]:
    """
    Fetch games for a specific league and date.
    
    Args:
        league_key: The league identifier (e.g., "EPL", "NBA")
    ...
    # Check if league is in season
    if not is_league_in_season(league_key):
        logger.info(f"{league_key} is not in season")
        return []
        
    # Get league info
    league_info = get_league_info(league_key)
    if not league_info:
        logger.error(f"Invalid league: {league_key}")
        return []
    ...
    if os.path.exists(cache_path):
        try:
            ...
            logger.error(f"Error reading cache for {league_key}: {e}")
    ...
    try:
        ...
        games = []  # Replace with actual API call
...
def fetch_all_games(date: Optional[str] = None) -> Dict[str, List[Dict]]:
    """
    Fetch games for all active leagues on a specific date.
    
    Args:
        date: Optional date string in YYYY-MM-DD format. Defaults to today.
        
    Returns:
        Dictionary mapping league keys to lists of game data
    """
    if not date:
        date = datetime.now().strftime("%Y-%m-%d")
    ...
    for league_key in active_leagues:
        games = fetch_games(league_key, date)
        if games:
            all_games[league_key] = games
