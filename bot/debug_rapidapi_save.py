...
async def test_rapidapi_save():
    ...
    try:
        ...
        async with MultiProviderAPI(pool) as api:
            ...
            logger.info(f"MultiProviderAPI db_pool available: {api.db_pool is not None}")
            ...
            if darts_leagues:
                ...
                if games:
                    ...
                    for game in games:
                        logger.info(f"Game data: {game.get('api_game_id')} - {game.get('home_team_name')} vs {game.get('away_team_name')}")
