...
async def verify_final_fix():
    ...
    try:
        ...
        for row in rows:
            print(f"  {row['api_game_id']}: {row['home_team_name']} vs {row['away_team_name']} - {row['league_name']}")
        
        # Check if there are any "Serie A" games with league_id=71 (should be 0)
        rows = await db.fetch_all('SELECT COUNT(*) as count FROM api_games WHERE league_id = "71" AND league_name = "Serie A"')
