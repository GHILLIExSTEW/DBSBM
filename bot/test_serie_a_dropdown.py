...
async def test_serie_a_dropdown():
    ...
    try:
        ...
        print(f"   Brazil Serie A games found: {len(brazil_games) - 1}")  # Subtract 1 for manual entry
        
        # Test Italian Serie A
        print("\n2. Testing Italian Serie A dropdown...")
        italian_games = await get_normalized_games_for_dropdown(db, 'SerieA')
        print(f"   Italian Serie A games found: {len(italian_games) - 1}")  # Subtract 1 for manual entry
