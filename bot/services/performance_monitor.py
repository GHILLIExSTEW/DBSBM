...
class PerformanceMonitor:
    ...
    def record_query(self, query: str, execution_time: float, success: bool = True,
                    error_message: Optional[str] = None, rows_affected: Optional[int] = None,
    ...
    def record_api_call(self, endpoint: str, method: str, response_time: float,
                       status_code: int, success: bool = True, error_message: Optional[str] = None,
                       cache_hit: bool = False) -> None:
        ...
        if response_time > self.thresholds['api_response_time_ms'] / 1000:
            logger.warning(f"Slow API call detected: {response_time:.3f}s - {method} {endpoint}")
    ...
    def _check_thresholds(self, metric: PerformanceMetric) -> None:
        ...
        if metric.name == "query_time" and metric.value > self.thresholds['query_time_ms'] / 1000:
            logger.warning(f"Query time threshold exceeded: {metric.value:.3f}s")

        elif metric.name == "api_response_time" and metric.value > self.thresholds['api_response_time_ms'] / 1000:
            logger.warning(f"API response time threshold exceeded: {metric.value:.3f}s")

        elif metric.name == "memory_usage" and metric.value > self.thresholds['memory_usage_mb']:
    ...
    async def log_performance_report(self) -> None:
        ...
        logger.info(f"Memory Usage: {report['system']['memory_usage_mb']:.1f}MB ({report['system']['memory_percent']:.1f}%)")
        logger.info(f"CPU Usage: {report['system']['cpu_percent']:.1f}%")

        query_stats = report['queries']
        logger.info(f"Database Queries: {query_stats['total_queries']} total, "
                   f"{query_stats['avg_time']:.3f}s avg, {query_stats['success_rate']:.1%} success rate")

        api_stats = report['api_calls']
        logger.info(f"API Calls: {api_stats['total_calls']} total, "
...
def record_api_call(endpoint: str, method: str, response_time: float, status_code: int,
    ...
    """Record API call performance."""
    performance_monitor.record_api_call(endpoint, method, response_time, status_code, success, cache_hit=cache_hit)
