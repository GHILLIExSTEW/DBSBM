...
class AnalyticsService:
    ...
    async def get_user_stats(self, guild_id: int, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            win_rate = (
                (wins / total_resolved_for_winrate * 100)
                if total_resolved_for_winrate > 0
                else 0.0
            )
            ...
            total_risked = (
                float(total_risked_result.get("total_risked") or 0)
                if total_risked_result
                else 0.0
            )
            ...
            logger.exception(
                f"Error getting user stats for user {user_id} in guild {guild_id}: {e}"
            )
    ...
    async def get_guild_stats(self, guild_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            total_resolved_for_winrate = wins + losses
            win_rate = (
                (wins / total_resolved_for_winrate * 100.0)
                if total_resolved_for_winrate > 0
                else 0.0
            )
    ...
    async def get_leaderboard(
        self, guild_id: int, metric: str = "net_units", limit: int = 10
    ) -> List[Dict]:
