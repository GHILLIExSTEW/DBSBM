...
class EnterpriseService:
    ...
    async def create_enterprise_config(
        ...
        try:
            ...
            # Create license
            await self._create_enterprise_license(
                tenant_id, license_key, license_type, features, max_users
            )
            ...
            await self.cache_manager.clear_cache_by_pattern(
                f"enterprise_configs:{tenant_id}"
            )
    ...
    async def get_enterprise_license(
        self, tenant_id: int
    ...
    async def check_feature_access(self, tenant_id: int, feature: str) -> bool:
        ...
        try:
            ...
            # Cache result
            await self.cache_manager.enhanced_cache_set(
                cache_key, has_access, ttl=self.cache_ttls["enterprise_features"]
            )
    ...
    async def add_enterprise_user(
        self, tenant_id: int, user_id: int, role: str, permissions: List[str]
    ) -> bool:
    ...
    async def get_enterprise_billing(
        self, tenant_id: int
    ) -> Optional[EnterpriseBilling]:
        ...
        try:
            ...
            analytics = {
                ...
                "license": {
                    ...
                    "expires_at": (
                        license_obj.expires_at.isoformat() if license_obj else None
                    ),
            ...
            # Cache analytics
            await self.cache_manager.enhanced_cache_set(
                cache_key, analytics, ttl=self.cache_ttls["enterprise_analytics"]
            )
