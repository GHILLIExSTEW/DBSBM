...
class NLPService:
    ...
    async def classify_intent(
        self, text: str, user_id: int, guild_id: int
    ) -> Optional[NLPIntent]:
    ...
    async def get_response(
        self, intent_type: IntentType, user_id: int, guild_id: int
    ) -> Optional[str]:
        ...
        try:
            ...
            result = await self.db_manager.fetch_one(
                query, {"intent_type": intent_type.value}
            )
            ...
            # Cache response
            await self.cache_manager.enhanced_cache_set(
                cache_key, response_text, ttl=self.cache_ttls["nlp_responses"]
            )
    ...
    async def process_message(
        ...
        try:
            ...
            response = (
                await self.get_response(intent.intent_type, user_id, guild_id)
                if intent
                else None
            )
            ...
            # Cache result
            await self.cache_manager.enhanced_cache_set(
                cache_key, result, ttl=self.cache_ttls["nlp_responses"]
            )
    ...
    async def get_analytics(self, guild_id: int, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            intent_results = await self.db_manager.fetch_all(
                intent_query, {"guild_id": guild_id, "days": days}
            )
            ...
            sentiment_results = await self.db_manager.fetch_all(
                sentiment_query, {"guild_id": guild_id, "days": days}
            )
            ...
            entity_results = await self.db_manager.fetch_all(
                entity_query, {"guild_id": guild_id, "days": days}
            )

            analytics = {
                "intent_distribution": {
                    row["intent_type"]: row["count"] for row in intent_results
                },
                "sentiment_distribution": {
                    row["sentiment_type"]: row["count"] for row in sentiment_results
                },
                "entity_distribution": {
                    row["entity_type"]: row["count"] for row in entity_results
                },
                "total_messages": sum(row["count"] for row in intent_results),
                "most_common_intent": (
                    max(intent_results, key=lambda x: x["count"])["intent_type"]
                    if intent_results
                    else None
                ),
                "most_common_sentiment": (
                    max(sentiment_results, key=lambda x: x["count"])["sentiment_type"]
                    if sentiment_results
                    else None
                ),
                "most_common_entity": (
                    max(entity_results, key=lambda x: x["count"])["entity_type"]
                    if entity_results
                    else None
                ),
            }
            ...
            # Cache analytics
            await self.cache_manager.enhanced_cache_set(
                cache_key, analytics, ttl=self.cache_ttls["nlp_analytics"]
            )
