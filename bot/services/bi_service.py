...
class BIService:
    ...
    async def start(self):
        ...
        try:
            ...
            self.report_generation_task = asyncio.create_task(
    ...
    async def get_reports_by_tenant(
        self, tenant_id: int, report_type: Optional[ReportType] = None
    ) -> List[BIReport]:
        """Get BI reports for a tenant."""
        try:
            # Try to get from cache first
            cache_key = (
                f"bi_reports:{tenant_id}:{report_type.value if report_type else 'all'}"
            )
    ...
    async def generate_report(
        ...
        try:
            ...
            # Cache report data
            await self.cache_manager.enhanced_cache_set(
                cache_key, report_data, ttl=self.cache_ttls["bi_reports"]
            )
    ...
    async def get_dashboards_by_tenant(
        self, tenant_id: int, dashboard_type: Optional[DashboardType] = None
    ) -> List[BIDashboard]:
    ...
    async def calculate_metric(
        self, metric_id: int, parameters: Optional[Dict[str, Any]] = None
    ) -> Optional[float]:
        ...
        try:
            ...
            # Cache metric value
            await self.cache_manager.enhanced_cache_set(
                cache_key, value, ttl=self.cache_ttls["bi_metrics"]
            )
    ...
    async def get_analytics(
        self, tenant_id: int, analysis_type: str, parameters: Dict[str, Any]
    ) -> Optional[BIAnalytics]:
        ...
        try:
            ...
            analytics_data = await self._generate_analytics(
                tenant_id, analysis_type, parameters
            )
