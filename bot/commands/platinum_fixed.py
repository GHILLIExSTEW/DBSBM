...
class PlatinumCog(commands.Cog):
    ...
    async def cog_app_command_error(
        ...
        if isinstance(error, app_commands.MissingPermissions):
            await interaction.response.send_message(
                "❌ You need 'Manage Server' permissions to use Platinum commands.",
                ephemeral=True,
            )
        else:
    ...
    @app_commands.command(
        name="platinum", description="Check Platinum tier status or upgrade"
    )
    ...
    async def platinum_status(self, interaction: Interaction):
        ...
        try:
            ...
            if is_platinum:
                # Get feature usage
                webhook_count = await self.bot.platinum_service.get_webhook_count(
                    guild_id
                )
                export_count = await self.bot.platinum_service.get_export_count(
                    guild_id, datetime.now().month
                )
                ...
                embed.set_footer(
                    text="Your Platinum subscription is active and ready to use!"
                )
        ...
        **Step 4: Test Your Webhook**
        ...
        try:
            ...
            instructions_embed.set_footer(
                text="💡 Tip: Use descriptive names like 'Bet Notifications' or 'Analytics Feed'"
            )

            await interaction.response.send_message(
                embed=instructions_embed, ephemeral=True
            )
            ...
            if webhook_type not in webhook_types:
                ...
                for wtype, description in webhook_types.items():
                    error_embed.add_field(
                        name=f"`{wtype}`", value=description, inline=False
                    )
            ...
            if success:
                ...
                success_embed.set_footer(
                    text="🎉 Your webhook is now active and receiving notifications!"
                )
    ...
    @app_commands.command(
        name="export", description="Export server data (Platinum only)"
    )
    @app_commands.checks.has_permissions(administrator=True)
    async def export_data(
        ...
        try:
            ...
            if success:
                ...
                if user:
                    embed.add_field(
                        name="User Filter", value=f"@{user.display_name}", inline=True
                    )
                else:
                    embed.add_field(name="User Filter", value="All Users", inline=True)
                embed.set_footer(
                    text="You will receive a notification when the export is ready."
                )
            else:
                embed = discord.Embed(
                    title="❌ Export Failed",
    ...
    @app_commands.command(
        name="analytics", description="View Platinum analytics (Platinum only)"
    )
    @app_commands.checks.has_permissions(administrator=True)
    ...
    async def list_webhooks(self, interaction: Interaction):
        ...
        try:
            ...
            for i, webhook in enumerate(webhooks, 1):
                webhook_type = webhook_types.get(
                    webhook.get("type", "unknown"), "❓ Unknown"
                )
    ...
    @app_commands.checks.has_permissions(administrator=True)
    async def remove_webhook(self, interaction: Interaction, webhook_name: str):
        ...
        try:
            ...
            class ConfirmWebhookRemoval(discord.ui.View):
                ...
                @discord.ui.button(
                    label="✅ Confirm Removal", style=discord.ButtonStyle.danger
                )
                async def confirm(
