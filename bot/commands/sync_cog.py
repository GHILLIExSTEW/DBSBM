        ...
        try:
            ...
            embed.add_field(
                ...
                value="\n".join([f"• `/{cmd}`" for cmd in synced_names[:10]]) + 
                      (f"\n... and {len(synced_names) - 10} more" if len(synced_names) > 10 else ""),
                inline=False
            )
            ...
            embed.set_footer(text=f"Guild ID: {interaction.guild_id} • Synced at {discord.utils.utcnow().strftime('%H:%M:%S')}")
        ...
        try:
            ...
            embed.add_field(
                ...
                value="\n".join([f"• `/{cmd}`" for cmd in synced_names[:15]]) + 
                      (f"\n... and {len(synced_names) - 15} more" if len(synced_names) > 15 else ""),
                inline=False
            )
            ...
            embed.set_footer(text=f"Global sync at {discord.utils.utcnow().strftime('%H:%M:%S')}")
        ...
        try:
            ...
            embed.add_field(
                ...
                value="\n".join([f"• `/{cmd}`" for cmd in guild_command_names[:20]]) + 
                      (f"\n... and {len(guild_command_names) - 20} more" if len(guild_command_names) > 20 else ""),
                inline=False
            )
