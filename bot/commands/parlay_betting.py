        ...
        try:
            ...
            if image_bytes:
                ...
                file_to_send = File(
                    ...
                    filename="parlay_preview_units.webp",
                )
            else:
        ...
        try:
            ...
            try:
                ...
                if image_bytes:
                    ...
                    preview_file = discord.File(
                        self.preview_image_bytes, filename=f"parlay_preview.webp"
                    )
                    embed = discord.Embed(
                        title=f"{len(self.bet_details['legs'])}-Leg Parlay Bet"
                    )
                    embed.set_image(url="attachment://parlay_preview.webp")
                else:
    ...
    async def go_next(self, interaction: Interaction):
        ...
        if self.current_step == 1:
            ...
            if self.preview_image_bytes:
                self.preview_image_bytes.seek(0)
                file_to_send = File(
                    self.preview_image_bytes, filename="parlay_preview.webp"
                )
    ...
    async def _handle_units_selection(self, interaction: Interaction, units: float):
        ...
        try:
            ...
            if image_bytes:
                ...
                self.preview_image_bytes.seek(0)
                file_to_send = File(
                    self.preview_image_bytes, filename="parlay_preview_units.webp"
                )
            else:
                logger.warning(
        ...
        try:
            ...
            if image_bytes:
                ...
                discord_file_to_send = File(
                    self.preview_image_bytes, filename=f"parlay_slip_{bet_id}.webp"
                )
            else:
                logger.warning(
