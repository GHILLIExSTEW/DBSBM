    ...
    def __init__(self, parent_view):
        ...
        for gender in ufc_fighter_service.get_gender_options():
            options.append(discord.SelectOption(
                label=gender,
                value=gender,
                description=f"Select {gender} fighters"
            ))
...
class UFCDropdownView(discord.ui.View):
    ...
    async def update_fighter_dropdown(self, interaction: discord.Interaction):
        ...
        self.add_item(UFCGenderDropdown(self))
        self.add_item(UFCCategoryDropdown(self, self.selected_gender))
        self.add_item(UFCFighterDropdown(self, self.selected_category, self.fighter_page))
    ...
    async def show_fighter_details(self, interaction: discord.Interaction, fighter_data: Dict):
        ...
        embed.add_field(
            name="Record",
            value=f"**{fighter_data['record']}** (W-L-D)",
            inline=True
        )
...
class UFC(commands.Cog):
    ...
    @app_commands.command(
        name="ufc_fighters",
        description="Browse UFC fighters by weight category"
    )
    ...
    @app_commands.command(
        name="ufc_search",
        description="Search for a UFC fighter by name"
    )
    async def ufc_search(self, interaction: discord.Interaction, query: str):
        ...
        if not results:
            await interaction.followup.send(
                f"❌ No fighters found matching '{query}'",
                ephemeral=True
            )
            return
