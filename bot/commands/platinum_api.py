...
class PlatinumAPICog(commands.Cog):
    ...
    async def _display_leagues_results(
        self, interaction: discord.Interaction, sport: str, result: dict
    ):
        """Display leagues query results."""
    ...
    async def _display_players_results(
        self, interaction: discord.Interaction, sport: str, result: dict
    ):
        """Display players query results."""
        players = result.get("response", [])
        if not players:
            # Get the parameters that were used for the query
            params_info = (
                f"League ID: {result.get('parameters', {}).get('league', 'Unknown')}\n"
            )
            params_info += (
                f"Team ID: {result.get('parameters', {}).get('team', 'All teams')}\n"
            )
            params_info += (
    ...
    async def _display_teams_results(
        self, interaction: discord.Interaction, sport: str, result: dict
    ):
        """Display teams query results."""
        teams = result.get("response", [])
        if not teams:
            embed = discord.Embed(
                title="🏈 Teams Query",
    ...
    async def _display_games_results(
        self, interaction: discord.Interaction, sport: str, result: dict
    ):
        """Display games query results."""
        games = result.get("response", [])
        if not games:
            embed = discord.Embed(
                title="⚽ Games Query",
                description="No games found for the specified criteria.",
                color=0x9B59B6,
            )
            await self._send_interaction_response(
                interaction, embed=embed, ephemeral=True
        ...
        # Use pagination if more than 10 results
        if len(games) > 10:
            view = GenericPaginatedView(self, games, sport, "games")
            await view.update_display(interaction)
        else:
            embed = discord.Embed(
                title=f"⚽ {sport.title()} Games",
                description=f"Found {len(games)} games",
                color=0x9B59B6,
            )
            for i, game in enumerate(games):
                ...
                status = fixture.get("status", {}).get("long", "Unknown")

                embed.add_field(
                    name=f"{i+1}. {home_team} vs {away_team}",
                    value=f"Score: {home_goals}-{away_goals}\nDate: {date}\nStatus: {status}",
                    inline=True,
                )
            await self._send_interaction_response(interaction, embed=embed)

    async def _display_odds_results(
        self, interaction: discord.Interaction, sport: str, result: dict
    ):
        """Display odds query results."""
        ...
        # Use pagination for odds (always paginated due to complexity)
        view = OddsPaginatedView(self, odds_data, sport)
        await view.update_odds_display(interaction)
    ...
    async def display_query_results(
        ...
        # Use query type-specific display functions
        display_functions = {
            "leagues": self._display_leagues_results,
            "players": self._display_players_results,
            "teams": self._display_teams_results,
            "games": self._display_games_results,
            "odds": self._display_odds_results,
        }

        display_function = display_functions.get(query_type)
        if display_function:
            await display_function(interaction, sport, result)
        else:
            embed = discord.Embed(
                title="❌ Unknown Query Type",
                description=f"Unknown query type: {query_type}",
                color=0xFF0000,
            )
            await self._send_interaction_response(
                interaction, embed=embed, ephemeral=True
