                ...
                try:
                    ...
                    if leagues:
                        ...
                        if leagues:
                            ...
                            logger.info(f"Testing game fetch for {test_league['name']}...")
                            
                            today = datetime.now().strftime("%Y-%m-%d")
                            games = await api.fetch_games(sport, test_league, today)
                            
                            if games:
                                logger.info(f"✅ Found {len(games)} games for {test_league['name']}")
                                ...
                                for game in games[:3]:  # Test first 3 games
                                    ...
                                    if success:
                                        saved_count += 1
                                        logger.info(f"  ✅ Saved game: {game.get('api_game_id')} - {game.get('home_team_name')} vs {game.get('away_team_name')}")
                                    else:
                                        logger.error(f"  ❌ Failed to save game: {game.get('api_game_id')}")
                                
                                logger.info(f"Database save test: {saved_count}/{min(3, len(games))} games saved successfully")
                            else:
                                logger.warning(f"⚠️ No games found for {test_league['name']} on {today}")
                    else:
            ...
            async with db_manager.db.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    for sport in missing_sports:
                        ...
                        if count > 0:
                            logger.info(f"✅ {sport.title()}: {count} games in database")
                        else:
                    ...
                    for sport in missing_sports:
                        ...
                        if recent_games:
                            ...
                            for game in recent_games:
                                logger.info(f"  {game[0]}: {game[1]} - {game[2]} vs {game[3]} ({game[4]}) - {game[5]}")
                        else:
