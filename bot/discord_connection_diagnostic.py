...
class DiscordConnectionDiagnostic:
    ...
    async def _check_network_connectivity(self):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                try:
                    async with session.get("https://discord.com", timeout=10) as response:
                        if response.status == 200:
                            logger.info("✅ Can reach Discord.com")
                            self.results['discord_connectivity'] = True
                        else:
                            logger.warning(f"⚠️ Discord.com returned status {response.status}")
                            self.results['discord_connectivity'] = False
                            self.warnings.append(f"Discord.com status: {response.status}")
    ...
    async def _check_discord_api(self):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                try:
                    async with session.get("https://discord.com/api/v10/users/@me", headers=headers, timeout=10) as response:
                        if response.status == 200:
                            data = await response.json()
                            logger.info(f"✅ Discord API test successful - Bot: {data.get('username', 'Unknown')}")
                            self.results['api_test'] = True
                            self.results['bot_username'] = data.get('username', 'Unknown')
                        elif response.status == 401:
                            logger.error("❌ Discord API returned 401 - Invalid token")
                            self.results['api_test'] = False
                            self.errors.append("Invalid Discord token")
                        else:
                            logger.warning(f"⚠️ Discord API returned status {response.status}")
                            self.results['api_test'] = False
                            self.warnings.append(f"Discord API status: {response.status}")
    ...
    def generate_report(self) -> str:
        ...
        for key, value in self.results.items():
            status = "✅ PASS" if value is True else "❌ FAIL" if value is False else "⚠️ UNKNOWN"
        ...
        if not self.results.get('api_test', False):
            report.append("3. Verify Discord token is valid in Discord Developer Portal")
        ...
        if not self.results.get('intents_configured', False):
            report.append("5. Ensure required intents are enabled in Discord Developer Portal")
