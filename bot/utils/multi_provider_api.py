...
ENDPOINT_CONFIGS = {
    ...
    "rapidapi-golf": {
        "golf": {
            "leagues": "/v1/events",
            "games": "/v1/events",
        },  # Golf uses events for both leagues and games
    },
    "rapidapi-darts": {
        "darts": {
            "leagues": "/tournaments-by-league",
            "games": "/matches-by-date",
            "seasons": "/seasons",
        },
    },
    "rapidapi-tennis": {
...
class MultiProviderAPI:
    def __init__(self, db_pool=None):
        ...
        if not os.getenv("API_KEY"):
            logger.warning(
                "API_KEY not found in environment variables - API-Sports features may not work"
            )
        if not os.getenv("RAPIDAPI_KEY"):
            logger.warning(
                "RAPIDAPI_KEY not found in environment variables - Golf features may not work"
            )
    ...
    async def make_request(
        ...
        provider = (
            ...
            else self.get_provider_for_sport(sport)
        )
        ...
        if provider_config["auth_type"] == "header":
            ...
            headers["x-rapidapi-host"] = provider_config.get(
    ...
    def _parse_rapidapi_darts_leagues(self, data: Dict, sport: str) -> List[Dict]:
        ...
        for league_obj in league_objects:
            ...
            if league_name and league_id:
                ...
                logger.info(
                    f"Added darts league: {league_name} (ID: {league_id}) with {len(tournaments)} tournaments"
                )
    ...
    async def fetch_golf_players(self, file_format: str = "json") -> List[Dict]:
        ...
        try:
            ...
            data = await self.make_request(
                "golf", "/get-player-list", params, provider_override="datagolf"
            )
    ...
    def _parse_rapidapi_golf_games(
        self, data: Dict, sport: str, league: Dict
    ) -> List[Dict]:
    ...
    def _parse_rapidapi_darts_games(
        self, data: Dict, sport: str, league: Dict
    ) -> List[Dict]:
    ...
    def _parse_rapidapi_tennis_games(
        self, data: Dict, sport: str, league: Dict
    ) -> List[Dict]:
    ...
    def _map_rapidapi_golf_game(
        self, event: Dict, sport: str, league: Dict
    ) -> Optional[Dict]:
    ...
    async def discover_all_leagues(self) -> Dict[str, List[Dict]]:
        ...
        logger.info(
            f"Multi-provider league discovery completed. Found leagues for {len(all_leagues)} sports"
        )
        return all_leagues
    ...
    async def fetch_all_leagues_data(
        ...
        logger.info(
            f"Starting multi-provider fetch for {date} and next {next_days} days"
        )
    ...
    async def _save_game_to_db(self, game_data: Dict) -> bool:
        ...
        logger.debug(
            f"Attempting to save game to database: {game_data.get('api_game_id')}"
        )
