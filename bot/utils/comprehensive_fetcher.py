...
class ComprehensiveFetcher:
    ...
    async def discover_all_leagues(self) -> Dict[str, List[Dict]]:
        ...
        async with LeagueDiscovery() as discoverer:
            ...
            total_leagues = sum(len(leagues) for leagues in self.discovered_leagues.values())
            logger.info(f"Discovered {total_leagues} leagues across {len(self.discovered_leagues)} sports")
    ...
    async def fetch_all_leagues_data(self, date: str = None, next_days: int = 2) -> Dict[str, int]:
        ...
        logger.info(f"Starting comprehensive fetch for {date} and next {next_days} days")
                ...
                try:
                    ...
                    for day_offset in range(next_days):
                        ...
                        if games_fetched > 0:
                            ...
                            logger.info(f"Fetched {games_fetched} games for {league['name']} on {fetch_date}")
    ...
    async def _fetch_league_games(self, sport: str, league: Dict, date: str) -> int:
        ...
        if sport == "football":
            ...
            endpoint = "fights"
        elif sport in ["tennis", "golf", "darts", "cricket", "boxing", "cycling", "esports", "snooker", "squash"]:
            endpoint = "matches"
        elif sport in ["futsal", "table-tennis", "badminton", "beach-volleyball", "field-hockey", "ice-hockey", "water-polo", "winter-sports"]:
            endpoint = "games"
        ...
        try:
            if sport == "football":
                ...
                return {
                    ...
                    "start_time": game.get("date", game.get("fixture", {}).get("date", "")),
                    "status": game.get("status", {}).get("long", ""),
        ...
        try:
            async with self.db_pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    await cur.execute("SELECT COUNT(DISTINCT league_id) as unique_leagues FROM api_games")
                    unique_leagues = (await cur.fetchone())["unique_leagues"]
                    
                    await cur.execute("SELECT COUNT(DISTINCT sport) as unique_sports FROM api_games")
...
async def run_comprehensive_hourly_fetch(db_pool: aiomysql.Pool):
    ...
    while True:
        try:
            ...
            logger.info(f"Waiting {wait_seconds/60:.1f} minutes until next comprehensive hourly fetch...")
            ...
            async with ComprehensiveFetcher(db_pool) as fetcher:
                ...
                logger.info(f"  - Failed leagues: {len(stats.get('failed_leagues', []))}")
