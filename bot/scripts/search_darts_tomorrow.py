#!/usr/bin/env python3
"""
Search for Darts matches tomorrow.
"""
        ...
        try:
            ...
            try:
                ...
                if matches:
                    ...
                    logger.info(f"✅ Found {len(all_matches)} unique matches for tomorrow")
                    
                    # Show match details
                    for j, match in enumerate(all_matches[:10]):  # Show first 10 matches
            ...
            logger.info("=" * 60)
            logger.info(f"Total Darts Leagues: {len(leagues)}")
            ...
            if all_matches:
                ...
                logger.info("😔 No Darts matches found for tomorrow")
                
        except Exception as e:
            logger.error(f"❌ Error searching for Darts matches: {e}")

async def main():
    """Run the search."""
    await search_darts_tomorrow()
