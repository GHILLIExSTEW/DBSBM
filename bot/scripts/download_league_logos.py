...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("league_logo_download.log"),
...
class LeagueLogoDownloader:
    ...
    async def make_api_request(self, sport: str, endpoint: str, params: Dict = None) -> Optional[Dict]:
    ...
    async def download_and_save_logo(self, sport: str, league_code: str, league_config: Dict):
        ...
        league_dir = os.path.join(self.logos_dir, sport_category.upper(), league_code.upper())
            ...
            try:
                ...
                async with self.session.get(logo_url) as response:
                    if response.status == 200:
                        ...
                        logger.error(f"Failed to download logo for {league_name}: HTTP {response.status}")
    ...
    async def download_all_logos(self):
        ...
        for sport, leagues in LEAGUE_CONFIGS.items():
            ...
            for league_code, league_config in leagues.items():
                ...
                success = await self.download_and_save_logo(sport, league_code, league_config)
                if success:
        ...
        logger.info(f"Download process completed. {successful_downloads}/{total_leagues} logos downloaded successfully.")
