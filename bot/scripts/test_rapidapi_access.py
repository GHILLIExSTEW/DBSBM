            ...
            try:
                ...
                async with session.get(
                    ...
                    if status == 200:
                        ...
                        logger.info(
                            f"{endpoint['name']}: SUCCESS - Got {len(str(data))} characters of data"
                        )
                    elif status == 403:
                        logger.error(
                            f"{endpoint['name']}: FORBIDDEN - API key doesn't have access to this service"
                        )
                    elif status == 401:
                        logger.error(
                            f"{endpoint['name']}: UNAUTHORIZED - Invalid API key"
                        )
                    elif status == 404:
                        logger.warning(
                            f"{endpoint['name']}: NOT FOUND - Endpoint doesn't exist"
                        )
                    else:
                        logger.warning(
                            f"{endpoint['name']}: Status {status} - {response.reason}"
                        )
