...
class MultiSportDataRecorder:
    ...
    def __init__(self, max_matches: int = None, batch_size: int = 50, delay_between_batches: float = 2.0):
    ...
    async def process_matches_in_batches(self, matches: List[Dict], sport: str) -> Dict:
        ...
        for i in range(0, len(matches), self.batch_size):
            ...
            logger.info(f"Processing {sport} batch {batch_num}/{total_batches} ({len(batch)} matches)")
            ...
            if i + self.batch_size < len(matches):
                logger.info(f"Waiting {self.delay_between_batches} seconds before next {sport} batch...")
    ...
    async def record_all_sports_data(self, sports: List[str], date: str) -> Dict:
        ...
        # Summary
        total_matches = sum(r.get('matches_saved', 0) for r in all_results.values() if isinstance(r, dict))
        ...
        logger.info(f"Success Rate: {((total_matches / (total_matches + total_errors)) * 100):.1f}%" if (total_matches + total_errors) > 0 else "N/A")
...
async def main():
    ...
    parser.add_argument("--sports", nargs="+", default=["tennis", "darts", "golf"],
