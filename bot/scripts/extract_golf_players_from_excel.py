...
def extract_golf_players_from_excel(excel_file_path: str):
    ...
    logger.info(f"Starting golf player photo extraction from Excel file: {excel_file_path}")
    
    # Load the Excel workbook
    try:
        workbook = openpyxl.load_workbook(excel_file_path)
        logger.info(f"Loaded Excel file with {len(workbook.sheetnames)} sheets: {workbook.sheetnames}")
    ...
    for i, img in enumerate(images):
        try:
            ...
            for search_row in range(max(1, row-2), min(sheet.max_row + 1, row+3)):
                for search_col in range(max(1, col-2), min(sheet.max_column + 1, col+3)):
                    ...
                    if cell_value and isinstance(cell_value, str):
                        ...
                        if len(cell_value.strip()) > 2 and ' ' in cell_value:
                            ...
                            if not player_name:
                                ...
                                logger.info(f"Found potential player name: {player_name}")
                            # Check if this looks like a league name
                            elif any(league_term in cell_value.upper() for league_term in ['PGA', 'LPGA', 'TOUR']):
            ...
            if not player_name:
                ...
                logger.warning(f"No player name found for image {i+1}, using: {player_name}")
            
            # Validate the image
            is_valid, metadata = is_valid_person_image(image_data)
            
            if not is_valid:
                logger.warning(f"Image {i+1} failed validation: {metadata.get('error', 'Unknown error')}")
            ...
            file_path = os.path.join(BASE_PHOTO_DIR, league, f"{clean_name}_excel_embedded_{timestamp}.png")
    ...
    logger.info(f"Extraction complete! Successful: {successful_extractions}, Failed: {failed_extractions}")
