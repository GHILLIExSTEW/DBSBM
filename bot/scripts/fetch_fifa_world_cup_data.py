...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            BASE_DIR / "logs" / "fifa_world_cup_fetch.log", encoding="utf-8"
        ),
    ],
)
...
class FIFAWorldCupFetcher:
    ...
    async def __aenter__(self):
        self.session = aiohttp.ClientSession(headers={"x-apisports-key": self.api_key})
        return self
    ...
    async def fetch_league_info(self) -> Optional[Dict]:
        ...
        if data and "response" in data and data["response"]:
            ...
            logger.info(
                f"Fetched league info: {league_info.get('league', {}).get('name')}"
            )
            return league_info
        return None
    ...
    async def fetch_teams(self, season: int = 2025) -> List[Dict]:
        ...
        params = {"league": WORLDCUP_CONFIG["league_id"], "season": season}
    ...
    async def fetch_seasons(self) -> List[Dict]:
        ...
        params = {"league": WORLDCUP_CONFIG["league_id"]}
...
async def insert_games_to_db(games: List[Dict], db_manager, season: int = 2025):
    ...
    logger.info(
        f"Inserting {len(games)} games into api_games table for WorldCup {season}"
    )
    for game in games:
        try:
            ...
            logger.info(
                f"Inserted/updated game {game_data['id']} ({home.get('name')} vs {away.get('name')})"
            )
        except Exception as e:
    ...
    async with FIFAWorldCupFetcher() as fetcher:
        ...
        if teams_2025:
            ...
            await save_data_to_json(
                {"teams": teams_2025}, "fifa_world_cup_teams_2025.json"
            )
        ...
        if standings_2026:
            await save_data_to_json(
                {"standings": standings_2026}, "fifa_world_cup_standings_2026.json"
            )
        ...
        if fixtures_2026:
            await save_data_to_json(
                {"fixtures": fixtures_2026}, "fifa_world_cup_fixtures_2026.json"
            )
        ...
        await save_data_to_json(
            {"fixtures": fixtures_2025}, "fifa_world_cup_fixtures_2025.json"
        )
