...
def is_high_quality_person_image(image_data: bytes) -> tuple[bool, dict]:
    ...
    try:
        ...
        if "Make" in exif_data:
            ...
            if any(
                brand in make
                for brand in ["canon", "nikon", "sony", "fujifilm", "leica"]
            ):
                quality_score += 3
            elif any(
                brand in make for brand in ["iphone", "samsung", "google", "pixel"]
            ):
                quality_score += 2
...
async def search_wikipedia_players(
...
async def get_wikipedia_page_images(
                ...
                for page_id, page_data in pages.items():
                    if "images" in page_data:
                        for img in page_data["images"]:
                            ...
                            if not img_title.startswith(
                                "File:"
                            ) and not img_title.startswith("Image:"):
                                continue
                            ...
                            async with session.get(
                                WIKIPEDIA_PAGE_URL, params=img_params
                            ) as img_response:
                                if img_response.status == 200:
                                    img_data = await img_response.json()
                                    img_pages = img_data.get("query", {}).get(
                                        "pages", {}
                                    )
                                                ...
                                                async with session.get(
                                                    ...
                                                    if download_response.status == 200:
                                                        ...
                                                        if is_valid:
                                                            ...
                                                            logger.info(
                                                                f"Found valid image: {img_title} (score: {metadata['quality_score']})"
                                                            )
...
async def download_golf_player_photos_strict():
    ...
    async with aiohttp.ClientSession() as session:
        for i, (player_name, player_data) in enumerate(players_dict.items(), 1):
            ...
            try:
                ...
                logger.info(
                    f"Found {len(page_titles)} Wikipedia pages for {player_name}"
                )
                ...
                logger.info(
                    f"Best image for {player_name}: score {best_metadata['quality_score']}"
                )
                ...
                file_path = os.path.join(
                    BASE_PHOTO_DIR, "PGA_TOUR", f"{clean_name}_strict_{timestamp}.png"
                )

                # Download the best image
                if await download_best_image(
                    session, best_image_url, file_path, best_metadata
                ):
                    logger.info(
                        f"Successfully downloaded high-quality photo for {player_name}"
                    )
    ...
    logger.info(
        f"Download complete! Successful: {successful_downloads}, Failed: {failed_downloads}, Quality Rejected: {quality_rejected}"
    )
