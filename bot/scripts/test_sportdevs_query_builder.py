...
def test_documentation_examples():
    ...
    print(
        f"   endpoint('players').property('first_name').insensitive.like('A*') -> {query}"
    )
    ...
    # and
    query = endpoint("matches").and_filter(
        lambda obj: obj.property("id").less_than(10).property("id").not_equals(1)
    )
    print(
        f"   endpoint('matches').and((obj) => obj.property('id').lessThan(10).property('id').not.equals(1)) -> {query}"
    )

    # or
    query = endpoint("matches").or_filter(
        lambda obj: obj.property("id").less_than(10).property("id").not_equals(1)
    )
    print(
        f"   endpoint('matches').or((obj) => obj.property('id').lessThan(10).property('id').not.equals(1)) -> {query}"
    )
    ...
    print(
        f"   endpoint('matches').order((obj) => obj.property('id').ascending) -> {query}"
    )

    # order descending
    query = endpoint("matches").order(lambda obj: obj.property("id").descending)
    print(
        f"   endpoint('matches').order((obj) => obj.property('id').descending) -> {query}"
    )

    # order with nullsFirst
    query = endpoint("matches").order(
        lambda obj: obj.property("id").ascending.nullsFirst
    )
    print(
        f"   endpoint('matches').order((obj) => obj.property('id').ascending.nullsFirst) -> {query}"
    )

    # order with nullsLast
    query = endpoint("matches").order(
        lambda obj: obj.property("id").descending.nullsLast
    )
    print(
        f"   endpoint('matches').order((obj) => obj.property('id').descending.nullsLast) -> {query}"
    )
    ...
    print(
        f"   Expected: matches?season_id=eq.18820&or=(away_team_score->current.gt.3,home_team_score->current.gt.3)&select=away_team_id,home_team_id,away_team_score,home_team_score&order=id.desc"
    )
