                ...
                if response.status == 200:
                    ...
                    for game in games:
                        try:
                            ...
                            logger.info(f"Saved game: {game_data['home_team_name']} vs {game_data['away_team_name']} ({game_data['start_time']})")
                            
                        except Exception as e:
                            logger.error(f"Error processing game: {e}")
                            continue
                    
                    logger.info(f"Successfully saved {saved_count}/{len(games)} games to database")
                    return True
