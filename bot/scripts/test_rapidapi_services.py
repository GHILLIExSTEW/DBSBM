                ...
                async with session.get(
                    ...
                    if status == 200:
                        try:
                            ...
                            logger.info(
                                f"{service['name']}: SUCCESS - Got {len(str(data))} characters of data"
                            )
                        except:
                            text = await response.text()
                            logger.info(
                                f"{service['name']}: SUCCESS - Got {len(text)} characters of text"
                            )
                    elif status == 403:
                        logger.error(
                            f"{service['name']}: FORBIDDEN - API key doesn't have access to this service"
                        )
                    elif status == 401:
                        logger.error(
                            f"{service['name']}: UNAUTHORIZED - Invalid API key"
                        )
                    elif status == 404:
                        logger.warning(
                            f"{service['name']}: NOT FOUND - Endpoint doesn't exist"
                        )
                    elif status == 429:
                        logger.warning(
                            f"{service['name']}: RATE LIMITED - Too many requests"
                        )
                    else:
                        logger.warning(
                            f"{service['name']}: Status {status} - {response.reason}"
                        )
