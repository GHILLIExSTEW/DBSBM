    ...
    if lock_file.exists():
        try:
            ...
            if lock_age.total_seconds() > 600:  # 10 minutes
                print(f"🗑️  Removing stale lock file (age: {lock_age.total_seconds()/60:.1f} minutes)")
                lock_file.unlink()
            else:
                print(f"⚠️  Lock file exists and is recent (age: {lock_age.total_seconds()/60:.1f} minutes)")
                print("   If the fetcher is not running, you can manually delete the lock file.")
