        ...
        try:
            ...
            async with session.get(url, headers=headers, params=params) as response:
                if response.status == 200:
                    ...
                    logger.info(
                        f"✅ Tournaments by league response: {len(data) if isinstance(data, list) else 'unknown'} tournaments found"
                    )
            ...
            async with session.get(url, headers=headers, params=params) as response:
                if response.status == 200:
                    data = await response.json()
                    logger.info(
                        f"✅ Seasons response: {len(data) if isinstance(data, list) else 'unknown'} seasons found"
                    )

                    # Handle both list and dict responses
                    seasons = (
                        data if isinstance(data, list) else data.get("seasons", [])
                    )

                    # Show first few seasons
                    for i, season in enumerate(seasons[:3]):
                        logger.info(
                            f"  {i+1}. {season.get('name', 'Unknown')} (ID: {season.get('id', 'Unknown')})"
                        )
                else:
