...
async def test_sportdevs_apis():
    ...
    async with MultiProviderAPI() as api:
        ...
        try:
            ...
            logger.info(f"Darts data keys: {list(darts_data.keys()) if isinstance(darts_data, dict) else 'Not a dict'}")
        ...
        try:
            ...
            logger.info(f"Esports data keys: {list(esports_data.keys()) if isinstance(esports_data, dict) else 'Not a dict'}")
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"Tennis data keys: {list(tennis_data.keys()) if isinstance(tennis_data, dict) else 'Not a dict'}")
        except Exception as e:
...
async def test_rapidapi_golf():
    ...
    async with MultiProviderAPI() as api:
        ...
        try:
            ...
            logger.info(f"Golf data keys: {list(golf_data.keys()) if isinstance(golf_data, dict) else 'Not a dict'}")
        except Exception as e:
...
async def test_apisports():
    ...
    async with MultiProviderAPI() as api:
        ...
        try:
            ...
            logger.info(f"Football API response: {len(football_data.get('response', []))} leagues")
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"Basketball API response: {len(basketball_data.get('response', []))} leagues")
        except Exception as e:
...
async def test_league_discovery():
    ...
    async with MultiProviderAPI() as api:
        try:
            ...
            logger.info(f"League discovery completed. Found leagues for {len(discovered_leagues)} sports")
            ...
            for sport, leagues in discovered_leagues.items():
                ...
                for i, league in enumerate(leagues[:3]):
                    logger.info(f"    - {league.get('name', 'Unknown')} (ID: {league.get('id', 'Unknown')})")
                if len(leagues) > 3:
            ...
            if leagues:
                ...
                try:
                    ...
                    if games:
                        ...
                        game = games[0]
                        logger.info(f"  Sample game: {game.get('home_team_name', 'Unknown')} vs {game.get('away_team_name', 'Unknown')}")
