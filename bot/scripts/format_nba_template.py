...
def format_nba_template():
    ...
    with open(
        "bot/data/basketball_nba_teams_template.json", "r", encoding="utf-8"
    ) as f:
        content = f.read()
    ...
    try:
        ...
        print(
            f"Successfully formatted NBA teams JSON with {len(data.get('response', []))} teams!"
        )
        print("File saved as: bot/data/basketball_nba_teams.json")
