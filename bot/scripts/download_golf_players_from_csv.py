    ...
    try:
        with open(csv_file_path, 'r', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                ...
                player_name = row.get('name') or row.get('player_name') or row.get('Name') or row.get('Player')
                image_url = row.get('image_url') or row.get('url') or row.get('Image') or row.get('URL')
                ...
                try:
                    ...
                    async with session.get(image_url, headers=headers) as response:
                        if response.status == 200:
                            ...
                            file_path = os.path.join(BASE_PHOTO_DIR, league, f"{clean_name}_csv_{timestamp}.png")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Download golf player photos from CSV file')
    parser.add_argument('csv_file', help='Path to the CSV file containing player data')
    parser.add_argument('--league', default='UNKNOWN', help='League name (e.g., PGA_TOUR, LPGA)')
